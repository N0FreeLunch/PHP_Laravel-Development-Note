...
토큰은 TOTP 화면에 엑세스 할 때, 아이디와 비밀번호를 입렿가는 페이지에서 넘어 왔는지 확인하기 위해 사용한 기능이다.

- 부정 엑세스 또는 로그인이 완료된 유저 : 서버(디비)에 저장된 토큰이 없을 때는 로그인 화면에서 넘어온 것이 아니기 때문에 부정 엑세스 또는 로그인이 완료된 유저이다.

- 입력 시간 초과 : 로그인 정보 대신에 엑세스 하는 것이기 때문에 지정한 입력 시간을 초과한 경우이다.

- 유효하지 않은 아이디 패스워드 : 로그인이 성공 했을 때 성공 정보를 토큰과 함께 저장하므로 

#### 로그인 어카운트와 함께 저장된 시크릿 정보에 의한 판별

- 2요소 인증 정보가 없는 경우 : 별도의 1대1 관계의 테이블에 2단계 인증을 위한 인증 정보가 존재하는 경우, 관계 테이블에 인증 정보를 저장한 레코드가 존재하지 않을 때

- 인증 시크릿 키가 없는 경우 : 계정과 함께 저장된 TOTP 코드의 일치 여부를 확인하는 시크릿 키가 없는 경우

- 인증 코드가 잘못된 경우 : 인증 코드는 어카운트 정보와 함께 저장되어 있는 시크릿 코드를 사용해서 인증 코드의 유효성 유무를 확인한다.

#### 벨리데이션 취약점 고려

해커에게 무엇이 원인이 되어 인증이 안 되는지 알려주는 것으로 인해서 무차별 대입 시도를 할 범위를 알려줄 수 있다. 아이디, 비밀번호, 인증코드 3가지 중에서 무엇 때문에 로그인이 되지 않는지를 알면 무차별 대입시도의 범위를 줄일 수 있다.

또한 유효한 아이디일 때만 락이 걸리는지 락이 걸리면, 락의 유무에 따라 아이디 패스워드 일치를 확인할 수 있기 때문에 락이 걸렸다는 메시지를 숨겨야 한다. 그래서 어떤 시도에 관해서든 '인증 코드가 올바르지 않거나, 인증 설정이 유효하지 않거나, 시도 횟수를 초과했다'는 메시지를 표시해서 무엇 때문에 인증되지 않는지를 최대한 알려주지 않는 편이 좋다.

하지만, 고객은 무엇 때문에 로그인 시도가 실패했는지 알고 싶어 할 것이다. 그래서 TOTP 코드를 입력하는 페이지에서 로그인을 시도한 아이디가 무엇인지 표시해서 잘못되었는지 확인 할 수 있도록 하는 등의 로그인 정보 잘못 입력에 대한 확인하게 한다. 또한 어카운트 락에 걸렸다면 메일을 보내서 락 걸린 상태를 알 수 있게 한다.

토큰에 의한 벨리데이션은 메시지 케이스별 메시지를 표시해도 문제가 없다. 하지만, 아이디, 비밀번호, 인증 번호에 의한 벨리데이션은 어떤 원인 때문인지 알 수 없게 숨기는 편이 좋다.
