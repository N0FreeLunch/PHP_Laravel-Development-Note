...
Trigger–Job–Unit 패턴은 ‘대상 선정(Trigger)–큐 실행(Job)–트랜잭션 단위 로직(Unit)’으로 책임을 분리하는 것이다.

Trigger–Job–Unitパターンは、「対象選定（Trigger）–キュー実行（Job）–トランザクション単位のロジック（Unit）」として責務を分離することです。
