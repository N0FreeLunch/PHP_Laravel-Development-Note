...
객체의 사양이 변경될 가능성이 있음에도 인터페이스를 만드는 이유는 테스트 객체를 만들어 실제 객체를 대체하는 동일 인터페이스가 구현된 가짜(fake) 객체를 만들어서 테스트를 쉽게 하기 위함이다. 하지만, 테스트를 할 때 주입의 통로 매개변수의 인터페이스 타입을 구현한 fake 객체를 만드는 것이나 Mockery와 같은 라이브러리를 사용하여 모의(Mocking) 객체를 만드는 것이나 비슷한 수고를 동반한다. 그래서 여러 클래스의 공통의 인터페이스를 정의할 필요가 없는 상황에서 테스트만을 위해서 인터페이스를 정의하는 것은 수정 변경을 어렵게 하므로 권장하지 않는다.

문제는 php 생태계에서 정적 분석의 용이성을 위해서 final 클래스를 만드는 것을 권장하고, final 클래스는 Mockery와 같은 라이브러리를 사용하여 모의(Mocking)할 수 없다는 점이다. 그래서 인터페이스를 정의하는 경우도 많은데, [bypass-finals](https://github.com/dg/bypass-finals)라는 라이브러리를 `composer require --dev dg/bypass-final`라는 명령어로 설치하면 개발 환경에서 final 키워드를 무시할 수 있기 때문에 런타임에서 동작하지 않는 정적 분석은 정적 분석대로 사용할 수 있고, 런타임에서 final이 무시되어 라이브러리 모킹을 사용해 테스트 할 수 있다.
