...
일반적으로 비밀번호 규칙은, 영어 대소문자 숫자를 반드시 한 문자 이상을 넣어 만들도록 강제한다. 특수문자를 입력할 수 있도록 한다. 일부 서비스에서는 특수문자를 반드시 한 문자 이상 포함하는 룰을 두기도 한다.

기본적으로 최소 12문자 제한을 걸도록 한다. 좀 더 길어도 좋다.
...
사람들을 비밀번호를 쉽게 기억하기 위해서 기존에 사용했던 비밀번호를 다른 사이트에서도 반복적으로 사용하는 경향이 있다. 이를 막기 위해서, 주기적으로 비밀 번호 교체를 하도록 강제하고, 이미 사용했던 비밀번호로 바꿀 수 없도록 강제하는 조치가 필요하다.

PCIDSS(결제 카드 산업 데이터 보안 표준) 요건에 따르면 과거 4번 바꾼 비밀번호와 동일한 비밀번호로 다시 바꿀 수 없도록 한다. 유저가 패스워드를 4번 바꾸고 나서 다시 자기가 기억하고 있는 비밀번호로 변경할 수 있기 때문에 일정 기간 동안 동일한 비밀번호를 사용했는지 중복을 체크하는 등의 방법을 생각해야 한다.

### 고려사항

평문 비밀번호로 해시 코드를 생성할 때, 항상 다른 해시 코드를 생성하기 때문에, 입력한 평문 비밀 번호를 해시화 한 값과 데이터베이스에 저장된 해시 코드를 직접 비교할 수 없으며, 특수한 알고리즘을 통해서 입력한 평문이 디비에 저장된 해시 코드와 일치하는지 확인해야 한다. 하지만 이 방식은, 모든 비밀번호 히스토리를 가져와서 하나씩 비교해야 하기 때문에 시스템 리소스를 많이 소비하는 단점이 존재한다.
...
과거 N 개월 동안 사용한 비밀번호의 데이터를 갱신한다.
