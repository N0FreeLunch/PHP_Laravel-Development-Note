...
현 회사에서 메인으로 담당한 프로젝트는 라라벨 스케줄러 기반으로 돌아가는 2년차 서비스였다. 기존 구조에서는 1분마다 크론이 실행되어 한 번에 단건씩 트랜잭션을 처리했는데, 전체 배치 처리 속도가 매우 느려 비개발자가 결과를 검증하기도 어려운 상황이었습니다.

더 큰 문제는, 서비스 규모가 커지면서 일부 배치 작업의 완료 시간이 5~10분까지 길어졌고, 크론이 1분마다 중첩 실행되다 보니 순간 서버에 5~10개의 배치가 동시에 동작하며 과부하로 서버가 다운되는 장애가 발생했다.

이 문제를 해결하기 위해, **배치 구조를 큐-잡 기반으로 전면 개편**하였다. 그 결과,
