...
- 배치를 만들 때는 스케쥴러, 큐, 잡 등의 이해와 이에 대한 코드를 짜는 것이 요구된다. 그에 반해 웹 개발자라면 MVC 패턴을 잘 알고 있고 로직을 쉽게 짤 수 있다. 또한 화면을 통해서 각종 설정과 옵션에 필요한 정보를 유저에게 받을 수도 있다는 장점이 있다. 물론 유저에게 UI를 제공해야 하는 불편함이 있지만 운이 좋다면 기존 UI에 버튼 하나 추가하는 것으로도 사용할 수 있다는 장점이 있다.
- 배치 처리는 각종 옵션을 코드에서 직접 넣어주어야 하기 때문에 배치가 동작하는 조건 설정에 까다롭고 화면에서 편하게 기능을 확인할 수 있는 것과는 달리 배치 처리는 스케쥴러에 의존하는 기능을 실행하기 위해 커멘드라인 등의 명령어를 만들어야 하는 등 더 많은 보일러 플레이트를 갖는 등의 문제를 가지기도 한다.
- 배치처리를 만드는 것은 이러한 까다로운 점들이 있기 배치처리를 만들기 전에 유저 인터페이스에서 처리하는 게 나은 기능인지 배치처리를 통해서 만들어야 하는 기능인지 미리 판단하는 것이 중요하다고 본다.

### 유저 인터페이스로 처리하면 좋은 작업
- 배치의 결과물을 생성하기 까지 많은 시간이 걸리지 않고 유저의 유저가 결과물을 얻기 까지 시간을 충분히 감당할 수 있는 경우, 큐잡을 통해서 비동기로 실행할 것이 아니라 유저의 화면에서 실행하여 그 결과에 대한 응답을 만들 수도 있다.
- 라라벨의 잡은 비동기 기능을 제공한다. 비동기 기능을 제공하기 때문에 유저에 의해서 코드가 실행될 때 오래 걸리는 작업에 대한 처리를 별도의 테스크로 진행하게 된다. 이를 통해 유저에게 빠른 응답을 줄 수 있다. 물론 유저는 비동기 프로세스가 완료될 때 까지 결과물을 기다려야 하고 해당 비동기 작업에 어느 정도의 시간이 걸릴지 정확히는 알 수 없고 경험을 통해서 어느정도 후에 작업이 완료되더라를 알 수는 있다.

### 배치 처리로 처리하면 좋은 작업
- 한 단위의 트랜잭션을 처리하는데 많은 시간이 걸리는 작업은 유저에게 불편함을 준다. 시스템 내부에서 미리미리 결과물을 생성하여 유저가 활동하고 있는 시간에는 결과물만 보여주어 유저의 편의를 돕고자 하는 방법이 필요하다.
- 특정 실행시점에 의존하는 정보들이 존재한다. 예를 들어 어떤 테이블의 정보는 계속 변하는데 특정 월에 실행하지 않고 나중에 실행하게 되는 경우, 시간이 지남에 따라 값이 달라진다면 처리 되어야 할 타이밍을 놓쳐 결과물이 잘못될 가능성이 존재한다. 이럴 때 미리미리 실행되어야 할 타이밍에 스케쥴러를 통해 배치처리를 실행하여 적절한 타이밍에 결과물을 얻는 과정은 배치처리로 처리하는 것이 좋다.  배치 처리에 필요한 모든 변경 정보들의 로그를 테이블에 잘 저장해 두었다면, 이 기록을 사용하여 결과물을 만들면 되기 때문에 굳이 스케쥴러로 특정 시점에서의 배치 처리를 하지 않아도 되는 장점이 있다.
...
#### 스프링 배치의 구조
- 스프링 배치의 `Job` 클래스는 `**ItemReader`메소드, `**ItemProcessor`메소드, `**ItemWriter`메소드를 사용한다. `ItemReader`는 데이터를 읽어들이는 역할을 한다. 데이터베이스에서 데이터를 가져올 수도 있고, 파일에서 데이터를 가져올 수도 있고, 기타 다양한 저장소에서 데이터를 하나씩 가공하는 구조이다.
- `ItemReader`는 데이터를 읽는 기능을 담당하며 이는 각종 저장소에 연결하는 기능들을 사용하기 때문에 이들에 의존적인 코드가 될 수 있다. `ItemProcessor`는 꺼낸 일정량의 데이터를 배치 처리를 하기 위한 적절한 구조로 변경하기 위한 가공 처리를 하는 과정이다. 만약 다양한 곳에서 데이터를 가져온다면 가능한 통일된 포멧으로 데이터를 변환시키는 역할을 한다.
- `ItemReader`와 `ItemProcessor`를 분리 시킨 것은 저장소에 따라 달라질 수 있는 데이터 취득 구조와 다양한 곳에서 온 데이터를 특정한 비즈니스 처리를 위해서 가공하는 과정을 분리하기 위해서라고 볼 수 있다. 저장소에서 데이터를 가져오는 로직은 A 비즈니스 로직에서는 a 포멧의 데이터로 B 비즈니스 로직에서는 b 포멧의 데이터로 데이터의 변환을 해야 하는 경우가 생길 수 있다. 각 비즈니스 로직의 목적에 맞게 데이터를 다양하게 가공하기 위한 것이 `ItemProcessor`의 목적이기 때문에 `ItemReader`와 분리하여 재사용성을 높이는 구조라고 할 수 있다.
- `ItemProcessor`를 통해서 특정 비즈니스 로직의 구현에 적절한 포멧의 데이터를 받아서 이를 비즈니스 로직에 따른 처리를 하는 곳이 `ItemWriter`부분에 해당한다. 일반적으로 비즈니스 로직에 따라 데이터를 처리하게되면 처리된 상태 또는 처리 결과 데이터를 데이터베이스에 저장하거나 파일과 같은 결과물을 만들어내기 때문에 `writer`이라고 표현한다.
- `ItemReader`와 `ItemProcessor`의 경우 데이터는 일반적으로 프로세스를 실행하는 컴퓨터의 메모리에 데이터가 저장된다. 어떤 결과물을 만드는 과정이 아니기 때문에 도중에 실패해면 해당 데이터는 메모리에서 날라가기 때문에 자연스런 롤백이 된다. 하지만 `ItemWriter`의 경우에는 실행 프로세스의 메모리가 아닌 외부의 저장소 또는 파일 등을 만들어내는 과정이기 때문에 실패했을 경우 중간 과정이 남지 않게 처리하는 트랜잭션의 과정이 필요하다. 따라서 스프링배치는 `Job`에서도 `ItemWriter` 부분에서 트랜잭션 처리를 한다.
- 라라벨 배치와 마찬가지로 각각의 잡은 테이블에 저장되어 큐에 의해 순차적으로 실행된다.
