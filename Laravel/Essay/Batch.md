...
실패한 잡은 리트라이 가능하며 리트라이 할 경우 리트라이 카운트가 1 증가한 동일한 테스크의 잡을 새로 큐에 추가하고 실패한 잡은 삭제한다. 이 과정을 지정한 리트라이 횟수만큼 반복하며 실패한 잡은 큐에서 제거되고 실패한 잡을 별도의 장소 (라라벨 디폴트는 failed_jobs 테이블)에 저장한다.

잡은 정상적인 동작의 완료 후 큐에서 제거된다. 잡을 실행하는 도중에 에러가 나거나 서버 문제가 생겨 실행이 끊기는 경우 큐에서 제거되지 않는다. 이 때 처리 도중에 문제가 발생하면 초기 상태로 돌아가서 다시 실행하는 전략이 베스트이기 때문에 한 단위의 잡은 트렌젝션으로 처리할 대상으로 묶는 개념을 생각하도록 하자.

라라벨에서 잡은 기본적으로 잡 클래스를 객체화하여 객체의 시리얼라이징 데이터를 저장소에 저장한다. 객체로서 저장되기 때문에 객체에 상태를 부여할 수 있고 객체의 프로터티에 잡이 동작할 옵션을 지정할 수 있다. 저장소에 저장되는 잡 단위마다 상태에 따른 다양한 동작을 지정할 수 있다.
