...
#### 스프링 배치의 구조
- 스프링 배치는 `ItemReader`, `ItemProcessor`, `ItemWriter`으로 구조를 가지고 있다. `ItemReader`, `ItemProcessor`는 데이터 단위 하나를 읽어들이는 역할을 한다. 데이터베이스에서 데이터를 가져올 수도 있고, 파일에서 데이터를 가져올 수도 있고, 기타 다양한 저장소에서 데이터를 하나씩 가공하는 구조이다.
- `ItemReader`는 데이터를 하나 꺼내는 과정이고 `ItemProcessor`는 꺼낸 하나의 데이터를 배치 처리를 하기 위한 적절한 구조로 변경하기 위한 가공 처리를 하는 과정이다. `ItemReader`, `ItemProcessor`를 통해 가공된 데이터를 일정량 쌓는 과정이 스프링 배치에서의 청크에 해당한다. 그리고 이 청크 단위로 `ItemWriter`를 실행하여 최종 결과물을 얻도록 한다.
- `ItemWriter` 쌓인 데이터를 청크 단위로 트랜잭션을 건다.
...
- `ItemReader`, `ItemProcessor` 통해서 동일한 구조의 데이터를 만들어 낼 수 있기 때문에 다양한 저장소로 부터의 데이터를 한꺼번에 청크 단위로 쌓아 배치처리 할 수 있는 구조를 가진다는 것이 장점이다.
...
- 스프링 배치와 비교를 하자면 `ItemReader`, `ItemProcessor`에 해당하는 것이 `JobTrigger` 부분에 해당한다. 그리고 `ItemProcessor`는 `Unit`에 해당한다고 볼 수 있다.
