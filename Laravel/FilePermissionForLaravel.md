...
권한 7은, 읽기(`100 = 4`), 쓰기(`010 = 2`), 실행(`001 = 4`)을 사용하는 코드이다. 이 중에서 소스 코드에 대한 쓰기 권한을 갖는 것은 위험할 수 있다. 해커가 웹 통신으로 접근을 하면 www-data 또는 nginx 유저의 권한으로 접근하게 되는데, 소스에 대한 수정 권한을 부여하면 취약점을 통해서 소스 코드를 변경할 수 있다. 따라서 www-data 또는 nginx 유저의 권한에 대해서는 소스 코드를 변경할 수 없도록 만드는 편이 좋다.
