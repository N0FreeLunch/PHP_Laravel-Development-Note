...
php엔진이 php 파일을 실행할 때는 파일의 코드를 읽어서 이를 실행한다. 따라서 php 파일을 그대로 실행을 하는 것이 아닌 php 파일을 읽고 실행은 php-fpm이 담당한다. 그래서 php 파일은 실행 권한 (`001 = 1`)이 필요하지는 않으며 읽기(`100 = 4`) 권한이 필요하다.
...
권한 7은, 읽기(`100 = 4`), 쓰기(`010 = 2`), 실행(`001 = 1`)을 사용하는 코드이다. 이 중에서 소스 코드에 대한 쓰기 권한을 갖는 것은 위험할 수 있다. 해커가 웹 통신으로 접근을 하면 www-data 또는 nginx 유저의 권한으로 접근하게 되는데, 소스에 대한 수정 권한을 부여하면 취약점을 통해서 소스 코드를 변경할 수 있다. 따라서 www-data 또는 nginx 유저의 권한에 대해서는 소스 코드를 변경할 수 없도록 만드는 편이 좋다.
