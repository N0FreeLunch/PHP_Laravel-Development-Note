...
## 스테이트리스 서버
- 웹 애플리케이션 서버의 중요한 가치는 서버는 스테이트리스(stateless)여야 한다는 것이다. 서버가 0대가 되지 않는다면 서버를 추가 및 제거해도 트레픽 처리 문제가 아니라면 동작에 문제가 없어야 한다는 것이다. 하지만 웹 서버는 데이터의 저장이 되어야 하고 여러 대의 서버끼리 데이터의 공유가 일어나야 한다.
