...
- 엘로퀀트를 사용하지 않고 쿼리 빌더만을 사용했을 때의 문제점은 릴레이션을 정의할 때 문제가 발생한다. join을 통해서 모델을 연결해야 하는데 하나의 모델에 다른 테이블과의 연결 관계를 모두 정의를 해야 한다. 맡은 테이블의 로직 뿐만 아니라 연관 테이블의 로직까지 정의해야 비 엘로퀀트 쿼리 빌더 방식으로 정의할 수 있다. 이러면 모델이 맡은 테이블의 책임만을 가지도록 만들어 나가기 어렵게 된다. 이에 반해 엘로퀀트를 사용하면 연관 테이블과의 관계를 엘로퀀트 모델을 사용하는 룰에 따라서 사용할 때 연관 관계를 간단하게 표현해서 사용할 수 있기 때문에 모델이 맡은 테이블의 로직만을 만들면 된다는 장점이 있어서 책임에 따른 클래스 설계를 하기에 용이하다.
...
### 테이블명
- 연관 테이블을 지정하지 않을 경우 : 엘로퀀트 모델 클래스 이름의 복수형을 스네이크 케이스로 변환한 이름이 사용 됨
...
```
- protected를 사용해서 primary key를 정의했다. 엘로퀀트 모델을 사용할 때 기본 키의 변경을 불가능하게 하기 위한 것이다.
...
```
- Eloquent는 기본적으로 primary key가 데이터베이스 테이블에서 설정된 auto increament 속성을 가지고 있다고 가정한다.
- auto increament가 프라이머리 키에 세팅되어 있지 않다면 엘로퀀트에 수동으로 `public $incrementing` 속성을 `false`로 설정하여 프라이머리 키 값이 자동으로 증가하는 값이 아니라고 알려줘야 문제가 생기지 않는다.
...
- 이 또한 기본키의 타입은 엘로퀀트 객체에서 변경이 되면 안 되기 때문에 protected으로 설정한다.
...
```
- 접근제한자가 다른 속성들이 protected로 설정된 것과는 달리 public으로 설정되어 있는데 이는 엘로퀀트 객체를 사용할 때, 타임스템프 사용 유무를 설정할 수 있도록 하기 위함이라고 생각 해 볼 수 있다. 하지만 객체지향의 원리에 맞게 엘로퀀트를 만들었다면 setTimestamp라는 메소드를 만들어서 사용했을 것이기 때문에 외부에서 timestamps의 설정 유무를 컨트롤 한다는 개념이 이상할 수도 있다. (이 부분은 실제로 확인을 해 봐야 할 듯)
...
```
- 라라벨의 마이그레이션 툴을 사용해서 테이블을 정의했다면 기본 보일러 플레이트에 created_at과 updated_at을 정의해 주고 있다는 것을 참고하자.
...
- 이런 기능의 제공은 인프라에 종속된 모델 로직을 만들지 않게 하기 위해서이다. 비즈니스 로직에서 사용되는 모델이 인프라의 구성이나 종류에 따른 영향을 최소화 하여 비즈니스 로직을 구성하는데 일관된 코딩 스타일을 제공하기 위한 것.
```
<?php
...
```
- 하나의 모델을 메모리에 불러오는 동안 일반적인 많은 라라벨 컬렉션 메소드를 사용할 수 있습니다(?)
...
- cursor는 빠르지만 메모리를 많이 소모한다. 여러번 쿼리를 디비에 전달해서 실행 결과를 받지 않고 한번에 데이터를 다 가져오기 때문에 많은 데이터를 버퍼에 저장하고 있어서 메모리 사용이 크다. 하지만 디비에서 데이터를 받는 과정이 한번 뿐이기 때문에 I/O 대기 시간이 한번 뿐이라 chunk 방식에 비해 굉장히 빠르게 처리한다.
...
- 라라벨의 쿼리 빌더에는 select 메소드와 addSelect 메소드가 있다. select 메소드는 쿼리에 select 절을 추가하는 메소드이고 addSelect는 추가적으로 조회할 컬럼을 지정하는 메소드이다.
- 단순 쿼리 빌더와 달리 엘로퀀트의 쿼리 빌더의 select와 addSelect 메소드는 서브쿼리 방식을 사용할 수 있는 기능을 제공한다.
- 서브쿼리 방식을 사용하기 위해서 select 메소드와 addSelect 메소드의 인자로 연관 배열을 넣는데, 키 값은 쿼리상의 컬럼(가상의 컬럼)을 지정하고, 벨류 값으로 엘로퀀트 모델을 지정한다. 
...
Flight::select('name')
    ...
    ->limit(1)
```
- Flight 엘로퀀트 모델은 flights 테이블에서 name 컬럼만 뽑는다. 이 때 조건은 목적지 아이디가 Flight 모델의 destination_id 컬럼이 Flight 모델에 설정한 Destination 테이블과 연관 관계 메소드인 destinations를 통해 연결된 Destination 모델의 id 컬럼의 값과 일치하는지의 여부를 컬럼 일치 여부를 확인하는 whereColumn 메소드를 통해서 확인한다. 이 때 서브쿼리로 보여주는 값은 단일 값이어야 하기 때문에 매칭된 값이 여러개 나올 수 있다. 그 때문에 하나만 선택해서 보여주기 위해서 limit로 제한을 하였다.

### 코드 해석
- Destination 엘로퀀트 모델에서 last_flight 컬럼 값이 Flight 엘로퀀트 모델에서 name 컬럼을 뽑은 대상을 넣어라는 의미

- Reference : https://stackoverflow.com/questions/36345690/how-addselect-builder-method-works-in-laravel

> 예를 들어, 우리는 목적지까지 가기 위해 destinations 테이블과 flights 테이블이 있다고 가정 해봅시다. flights 테이블에는 항공편이 목적지에 도착한 시점을 나타내는 arrived_at 열이 있습니다. 서브쿼리 메소드 중에 select와 addSelect는 단일 쿼리를 사용하여 가장 최근에 목적지에 도착한 항공편의 이름과 목적지를 모두 선택할 수 있습니다.

- addSelect : https://laravel.com/api/8.x/Illuminate/Database/Query/Builder.html#method_addSelect
- 
