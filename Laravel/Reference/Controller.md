...
- 간단한 서비스에서는 컨트롤러에 비즈니스 로직을 넣고 디비의 데이터를 입출력하는 모델만 분리한다. 하지만, 로직이 커지면 서비스 레이어로 로직을 분리하고 컨트롤러에는 리퀘스트를 처리하여 서비스 레이어로 데이터를 보내주는 역할, 서비스레이어의 결과를 받아서 리스폰스를 처리하는 로직만들 넣어준다. 최대한 컨트롤러를 가볍게 만드는 것이 중요하다.
