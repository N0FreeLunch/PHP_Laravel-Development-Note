...
### 비즈니스 로직의 처리를 하는 부분
- 클래스는 일반적으로 단일 책임의 원칙을 가져야 한다. 컨트롤러로 만드는 클래스의 경우, 단일 책임을 지고 있지 않다. 컨트롤러에서 클래스의 메소드는 각 리퀘스트에 대한 처리를 하는 담당하고 있다. 컨트롤러 클래스는 어떤 로직의 동작을 자체 클래스의 메서드의 조합으로 동작시키지 않는 것이다. 이를 클래스 답지 않다고 하자.
- 클래스 답지 않은 컨트롤러 클래스의 메소드는 클래스의 부분으로서 어떤 기능을 담당하는 것이 아닌 컨트롤러 클래스가 정의된 개념적 단위 내에서 메서드 별 독립 된 단위를 가지고 있기 때문에 메서드라는 명칭 보다는 엑션이라는 명칭으로 불린다. 비즈니스 로직의 의미론적인 한 단위, 아니면 디비의 데이터 처리에 관한 로직인 CURD를 모아 놓은 한 단위로서 구성이 된다.
- 보통 CURD의 구성은 하나의 테이블에 대한 CURD를 처리하는 것이다. 그리고 컨트롤러의 메서드도 각 테이블의 CURD에 대한 처리를 담당하도록 구성하는 것이 기본적이다. 
- 컨트롤러의 로직이 복잡해질 경우, 따로 비즈니스 로직에 대한 처리를 하는 서비스 레이어를 만들어서 처리한다. 서비스 레이어의 각 클래스는 하나의 엑션에 대한 서비스 로직을 담당하도록 만든다. 예를 들어 C, U, R, D를 나탄내는 컨트롤러의 엑션 'index, create, store, show, edit, update, destroy' 각각에 대해서 서비스 레이어에서는 각각의 클래스로 처리 로직을 만드는 식으로 만드는 것이다.
- 간단한 서비스에서는 컨트롤러에 비즈니스 로직을 넣고 디비의 데이터를 입출력하는 모델만 분리한다. 하지만, 로직이 커지면 서비스 레이어로 비즈니스 로직을 분리하고 컨트롤러에는 리퀘스트를 처리하여 서비스 레이어로 데이터를 보내주는 역할, 서비스레이어의 결과를 받아서 리스폰스를 처리하는 로직만들 넣어준다. 최대한 컨트롤러를 가볍게 만드는 것이 중요하다.
...
## 의존성 주입 & 컨트롤러
- 컨트롤러에서는 생성자 주입을 하는 코드를 직접 사용하지 않는 것이 표준이다.
- 라라벨의 IOC에 의해 객체는 자동으로 의존성 주입이 되며, 라우터에 의해 컨트롤러가 실행이 될 때, 라라벨 프레임워크에 의해 의존성을 주입된다.
- 이 때 라라벨의 IOC에 의한 자동 의존성 주입을 위해서 생성자나 메서드에 타입힌트를 사용하여 주입할 클래스를 지정 해 줘야 한다. 
- 라라벨에서 사용되는 객체에 의존성 주입을 하고자 할 때는 라라벨의 서비스 프로바이더를 통해서 IOC 방식의 동작으로 자동으로 의존성 주입을 하는 기능을 추가할 수 있다. 클래스 내에 직접 `new SampleClass(InjectedObject)`를 사용하지도 않아도 외부 객체를 집어 넣는데 이를 의존성 주입으로 한다.
- 일반적으로 의존성을 주입하기 위해서는 `app\Providers` 폴더의 서비스 프로바이더에 자동 주입을 하기 위한 설정을 해 줘야 한다. 컨트롤러에 대한 IOC에 의한 의존성 주입은 이 부분에 정의되어 있지는 않은 것으로 보이며, 라라벨 프로젝트에서 제공하는 것으로 보인다. (이 부분에 대해서는 더 정확하게 조사 후 추가)
- 라라벨 프레임워크는 컨트롤러에 자동으로 의존성 주입을 한다. 컨트롤러가 `new SampleConatoller(InjectedObject)` 이런 코드 없이 의존성 객체를 매개변수로 받을 수 있는 이유이다. 이 코드에서 컨트롤러에 InjectedObject 객체를 주입해야 하는데, 직접적으로 주입하는 코드(`new SampleConatoller(InjectedObject)`)를 사용하지 않을 경우 InjectedObject를 생성자나 메소드의 매개변수를 통해 주입하기 위해서 컨트롤러의 생성자나 메소드에 주입할 오브젝트의 클래스를 타입힌트로 하는 매개 변수를 적는다.
...
```
### 인풋
- 컨트롤러 메서드의 매개 변수로 리퀘스트에 대한 정보를 받는다.
- 라우터의 URL param을 변수 파라메터로 받는 방식을 사용한다.
- 리퀘스트에 관한 모든 정보는 Request 객체를 파라메터를 통해 주입 받는 방식으로 사용한다.
