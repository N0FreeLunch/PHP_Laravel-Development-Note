...
- 실제 원본 클래스/객체를 사용하는 것이 아니라 원본 객체의 메서드를 연결시켜주는 프록시 메서드를 사용하므로 프록시 메서드를 자유롭게 조작을 할 수 있다면 파사드에 대한 태스트를 작성할 수 있다. 원본 객체의 메서들를 직접 사용하는 것이 아니라 파사드 프록시를 통해 사용하기 때문에 파사드를 호출하여 여러 변경을 적용할 수 있어서 다이나믹 메서드라고 표현한다.
