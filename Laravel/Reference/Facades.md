...
- 서비스 컨테이너에 선언되어 있는 것은 부트스트래핑 동안에 만들어지므로 부트스트래핑 과정에 포함되지 않는 어플리케이션 영역에서 파사드를 사용할 수 있다.
...
#### 파사드의 헬퍼함수와의 비교
- 파사드를 호출하기 위한 네임스페이스로 접근하도록 `use Illuminate\Support\Facades`이런 코드를 작성해야 한다.
- 파사드는 단일 함수로 제공되는 것이 아니라, 객체를 정적 멤버 형태로 사용하기 때문에 메서드를 체이닝하거나 파사가 연결해 준 객체의 상태변경을 통한 논리를 전개할 수 있다.
```
