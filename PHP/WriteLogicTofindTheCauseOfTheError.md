...
코드에 어떤 타입인지 명확하게 체크하는 로직을 만든다. `is_null`, `is_int`, `is_float`... 등의 타입 체크 기능을 이용하여 코드를 작성해서 잘못된 타입이 들어가서 의도하지 않은 연산을 하게 될 여지를 줄인다. 이를 통해서 다른 부분의 코드를 살피지 않고 직접 디버깅을 하지 않더라도 어떤 타입을 갖는지 명확하게 알 수 있도록 한다. 코드를 짤 때 어떤 타입의 값이 할당되는지 명확하게 알 수 있기 때문에 로직의 추가 변경이 수월해진다.
