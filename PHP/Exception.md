...
예외는 처리되지 않으면 최종적으로는 프로세스를 정지시킨다. 프로세스를 정지하는 것은 처리되고 있던 상태를 날려버리거나 처리의 되어야 할 데이터의 일부에 결함이 생기는 것이기 때문에 정지되지 않도록 만들 필요가 있다. 하지만, 프로그램을 작성하다 보면, 예외 처리를 해야하지만 미처 예외 처리를 하지 못하는 경우도 있고, 어떤 라이브러리의 기능을 사용했는데 충분히 사양서를 읽지 않아 예외가 던져지는 줄 몰랐을 경우도 있고, 동적 언어인 php는 타입 문제로 에러가 던져질 수도 있는 등 해야 했지만 하지 못한 실수들이 발생할 수 있다. 이런 경우 처리되지 않은 예외라면 최종적으로는 프로세스의 처리를 멈추고 예외를 개발자에게 보고하여 왜 예외가 발생했고 예외가 발생하지 말았어야 했는데 예외가 발생해서 처리 로직을 바꿔야 할지, 예외를 처리해야 했는데 처리를 누락해서 처리 로직을 추가 해 줘야 할지를 판단해서 예외가 발생하지 않도록 만드는 것이 올바른 방법이다.

#### 에러와 예외의 차이

기본적으로 예외는 처리 되는 것을 가정하고 만들어지며, 에러는 발생하지 않는 것을 가정한다. 정수 연산에서 정수의 범위가 초과한 케이스, 0으로 나눈 케이스, 타입힌트와 값의 타입이 불일치하는 경우 에러로 처리 되는데 이런 케이스는 발생하지 않도록 프로그래밍을 해 줘야 한다. 정수의 연산의 범위가 범위를 초과하지 않도록 코드를 짜 줘야 하며, 0으로 나눠지지 않도록 0으로 나눠지지 않도록 코드가 짜여야 하며, 전달되는 값이 타입힌트의 지정한 타입이어야 한다. 이런 에러는 예외 처리의 대상이 아니라 프로세스의 실행을 멈추는 것이 마땅하며, 프로그래머는 에러가 발생하지 않게 코드를 짜 줘야 한다. 예를 들어 분기 처리를 할 때 특정 분기에 대해서는 코드가 실행되지 않는 케이스가 존재할 수 있다. 이럴 때는 예외를 던지기 보다는 에러를 던져서 처리를 멈추고, 프로그래머는 보고된 에러가 발생하지 않는 로직을 만들어 줘야 한다.

#### 예외는 처리되는 것을 기대한다.

에러가 해당 상황이 일어나지 않도록 해야 하는 것인데 반해, 예외는 일어날 수 있는 상황에서 발생 시킨다. 예외는 별도의 처리가 없다면 프로세스를 종료시키므로 반드시 처리를 하여 프로세스가 정지되지 않도록 해 줘야 한다. 어떤 기능을 사용했는데 문제가 생겼다. 그럼 해당 문제가 해결되지 않은 상태로 로직의 흐름을 이어 나갈 수 있도록 해야 한다. 그러므로 예외는 반드시 처리 되는 것을 기대하며 누군가 에러가 아닌 예외를 던지는 코드를 만들었다면 예외를 반드시 잡아서 처리를 해 줘야 하는 의도가 깔린 것이라고 봐야 한다.

예외는 기본적으로 잡아서 처리해 주는 것이 의도이므로, 만약 어떤 기능에서 예외를 발생 시키는 코드가 있는데, 코드는 있지만 실제 조건이 맞지 않아 실행되지 않는 예외라면, 해당 예외를 잡아서 에러로 다시 던져 처리를 정지하는 등의 코드라도 추가하는 편이 낫다. 로직 상에서 의도적으로 예외를 잡을 필요가 없다는 것을 드러내기 위해서이다.
