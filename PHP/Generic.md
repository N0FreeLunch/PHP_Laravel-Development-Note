...
반면 PHP는 런타임에서도 모든 값의 타입이 유지되기 때문에, var_dump()와 같은 도구를 통해 실시간으로 타입을 확인하고, 어디서 타입이 예상과 달라졌는지를 쉽게 파악할 수 있습니다. 즉, PHP는 실행 중 발생하는 타입 오류를 통해 문제 지점을 명확히 확인할 수 있어, 타입 추적과 디버깅이 상대적으로 수월하다는 장점이 있습니다.
...
하지만 `@var array<int, string> $list`라는 docblock을 사용하면 IDE는 타입을 배열의 원소 타입을 알 수 있고, `assert`로 배열의 원소 타입을 런타임에서 체크하도록 하엿다. 이를 통해서 IDE와 정적 분석에 의한 타입 확인 및 런타임 타입 확인 둘을 만족하는 코드를 작성할 수 있어서 배열을 순회 할 때마다 `assert`로 각 원소의 타입을 확인할 필요가 없다.
