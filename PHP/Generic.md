...
foreach文では `$el` の型が分からないため、`echo $item` で出力できる文字列への変換が可能かどうかも分かりません。そのため、`assert` を使って各要素の型が文字列かどうかを確認する必要があります。
...
PHPだけでビジネスロジックを構成し、サービスを提供することに問題はありません。しかし、最近は多くの言語が優れた型システムを導入しているため、良い型システムを学べる他の言語もあわせて活用できる開発環境が望ましいでしょう。
