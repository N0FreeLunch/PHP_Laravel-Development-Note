...
위의 코드는 phpstan, psalm과 같은 강력한 정적 분석 도구에 의해서는 문자열 원소에 대한 처리를 하지 않았다는 에러 메시지가 발생하는데, `array_reduce` 등으로 사전조건을 지정하는 경우, 배열의 모든 원소를 확인하는 약간 복잡한 코드가 들어간 `assert`문의 타입 제한은 정적 분석 도구가 분석하지 못하기 때문에 `$list`의 원소가 `int`만이 전달된다는 사전 조건을 지정하더라도 여전히 `int`와 `string` 모두에 대한 처리를 해야 한다고 지적한다는 문제점이 있다.
...
IDE는 유니온, 공통 인터페이스, 상위 클래 등의 타입의 변수를 처리할 때, 해당 타입이 가진 모든 타입에 대한 처리를 하지 않으면 타입 미스매칭으로 지적한다. 타입 좁히기를 사용하면 여러 타입으로 구성된 것들 중 특정 타입에 대해서 성립한다는 전제를 `assert`를 통해서 IDE에 알려주어 원래 타입으로 지정된 것은 여럿이지만 실제 이 로직에서 사용할 것은 이 타입이기 때문에 이 타입에 대해서만 다음 로직의 처리가 맞는지 확인해 줘라는 의미로 사용할 수 있다.
...
php만으로 비즈니스 로직을 구성하고, 서비스를 제공하는데 문제는 없다. 하지만, 최근 많은 언어들이 좋은 타입 시스템을 도입하고 있기 때문에 다른 언어를 병행하는 경험을 제공하는 것이 좋다고 본다. 최소한 타입스크립트 정도는 병용하여 기본적으로 알아야 할 언어에 대한 지식을 습득시키는 것이 좋지 않을까?
