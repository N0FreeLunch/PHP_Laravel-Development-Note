...
하지만, 실제 전달되지도 않는 여러 타입에 대한 로직 처리를 할 필요는 없기 때문에 타입힌트로 지정한 타입 모두에 대한 처리가 아닌 일부 타입에 대해서만 처리 로직을 만드는 경우가 생긴다. 기본적으로는 지정한 타입 모두에 대한 처리를 하는 것이 일반적이다. 예를 들어 타입힌트가 `?int`으로 되어 있다면 연결되는 로직은 `int`와 `null` 모두에 대한 처리 로직을 만들어야 하며, `null`이 실제로 사용하지 않는 값이라면 `?int`가 아닌 `int`만 사용하여 타입힌트에서 제거하는 것이 해당 코드를 사용할 때 불필요한 처리를 추가로 작성하는 것을 방지한다.

유니온 타입이나 mixed 타입을 사용하면, 타입힌트로 지정한 타입 모두에 대한 처리가 아닌 일부에 대한 처리하도록 코드를 만들 수 있지만, IDE에 의한 정적 분석 또는 정적 분석 도구의 확인에 의해 에러로 지적되는 부분이 생긴다.

php에서는 타입힌트로 사용하고자 하는 타입의 내부 타입을 설정하는 제네릭 문법이 없기 때문에, 기본적으로는 여러 타입에 대해서 성립하는 로직을 만들되, 값을 전달할 때 또는 값을 꺼내 쓸 때 타입을 확인하는 방식을 사용해야 한다.
