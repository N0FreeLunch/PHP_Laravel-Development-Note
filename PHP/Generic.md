...
위의 코드는 phpstan, psalm과 같은 강력한 정적 분석 도구에 의해서는 문자열 원소에 대한 처리를 하지 않았다는 에러 메시지가 발생하는데, 배열의 모든 원소를 확인하는 약간 복잡한 코드가 들어간 `assert`문까지는 정적 분석 도구가 분석하지 못하기 때문에 `$list`의 원소가 `int`만이 전달되더라도 여전히 `int`와 `string` 모두에 대한 처리를 해야 한다고 에러를 발생시킨다.
