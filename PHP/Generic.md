...
위와 같은 배열이 어떤 알고리즘으로 전달되었을 때 이 배열이 어떻게 변환되어 반환될지 예측하기 어렵다.
...
일반적으로 제네릭은 다양한 타입을 지정할 수 있는데, 보통 제네릭은 사용하는 코드는 지정한 타입에 대해 타입별로 분기처리를 하는 로직이 아니라, 모든 타입에 대해 동일하게 적용되는 코드를 만드는 것이 일반적이다.
