...
문제는 런타임에서 타입을 확인하지 않기 때문에, 런타임에 잘못된 값이 전달되지 않게 하기 위해 지나치게 엄격하게 타입을 지정해야 하는 문제가 생긴다. 개발 초기 단계 부터 엄격하게 타입의 정적 추론이 가능하도록 짜 놓은 프로젝트인 경우 런타임의 타입 검사를 배제해도 잘못된 타입이 들어가는 경우를 막을 수 있지만, 정적 추론 없이 오랫동안 작성되어 온 애플리케이션이라면 정적 추론을 위해 모든 데이터 전달 플로우에 타입을 넣는 것이 현실적으로 어려운 문제에 직면할 수 있다.
