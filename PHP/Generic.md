...
만약 제네릭을 도입하게 되더라도 더 이상 php라고 할 수 없을 정도의 변경사항이 발생할 수도 있다는 읙견이 있다. php는 레거시 코드에 대한 최대한의 호환성을 유지하면서 업데이트가 되는 전략을 가지고 있으므로 제네릭 도입으로 기존의 코드를 너무 많이 수정해야 한다면, 에러가 발생하게 된다. 
