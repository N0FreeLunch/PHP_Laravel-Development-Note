...
### 캡슐화 이용하기
- Dto를 이용하고 싶지 않다면 캡슐화를 이용하는 방법이 있다. 메소드를 통해서 특정한 유형의 값을 받고, 해당 값을 멤버에 저장하며, 값을 가져오고 싶을 때는 배열의 구조와 형태를 추측할 수 있는 메소드명을 호출하는 방식을 사용하는 것이다. 데이터의 형태를 컨트롤 하는 객체를 만들어, 해당 객체를 통해서 데이터를 다루는 방식을 사용하는 것을 통해서, 캡슐화 된 방식이 아닌 방식으로 다루는 것을 최대한 배제하는 방식이다.
```php
