...
- 데이터가 할당되지 않았다는 의미로 null을 사용하는 것이 일반적이며, null을 하나의 값으로 취급하지 않도록 코딩에 주의해야 한다.
- null은 데이터가 할당되었는지 할당되지 않았는지를 체크하는 의미로 사용되며, 반드시 실행되어야 하는 로직에 선행하는 단계에서 요구되는 요청이 
