...
- `assert` 부분의 코드를 제외해 보자. 그러면 일반적으로 코드를 짜는 것과 동일하다는 것을 알 수 있다. 프로덕션 환경에서는 `assert`가 제외된 코드가 실행될 것이다. Assert는 개발자가 로직에 대해서 사전 조건과 사후 조건을 파악하는데만 이용할 뿐, 실제 코드의 동작에 영향을 주는 요인이 되지 않는 코드의 표현으로서 고려되어야 할 가치는 없지만, 코드의 동작 범위를 설정함으로써 이해를 돕기 위한 용도로 사용하고 있다는 것을 알 수 있다.
...
#### 단점
- 하지만, Assert는 런타임에 확인하는 동작이므로 새로운 기능을 추가한 사람이 해당 로직에 대한 충분한 실행을 했을 때 감지할 수 있으며, 가끔 코드를 충분히 실행하지 않을 때도 있기 때문에 Assert가 버그를 완전히 잡아준다는 기대를 할 수는 없다. 왜냐면 Assert는 실제 환경에서는 무시되는 코드이기 때문에 개발 환경에서 충분한 경우의 수의 로직을 실행하지 못한다면 QA도 런타임에서도 포착하지 못하는 케이스의 에러가 발생할 수 있다.
...
- 프로덕션에서 사용할 코드라면 굳이 Assert라는 것을 사용할 필요가 있는지를 생각해 볼 수 있을 것이다. 그냥 커스텀 예외를 만들면 되는 것 아닌가라는 생각을 할 수 있다. Assert가 커스텀 예외/에러를 만들어 던지는 것에 비해 간결한 문법을 갖고 있기 때문에 Assert가 커스텀 예외/에러를 대체할 수 있을 거라 생각해 볼 수도 있을 것이다.
...
### 예외 처리를 쓸 곳과 쓰지 않을 곳을 고려하기
- 예외처리를 만드는 것은 번거로운 일이다. 예외 처리를 다루기 위한 코드가 존재하는 곳에서는 예외를 사용하는 편이 좋지만, 예외 처리를 굳이 사용하지 않는 부분에서 조건 확인을 위해서 예외를 발생 시키는 것은 코드가 지저분해질 뿐 아니라 코드의 복잡성을 증가시킨다.
- 런타임에서의 동적인 값 할당이 없는 것이 명확한 코드인 경우, 런타임에서 발생하지 않을 예외를 만들기 보다는 assert를 사용하는 편이 좋다. 이 때 추후의 약간의 코드 변경이 일어나더라도 런타임에서 동적인 값의 할당이 일어나지 않는다는 것을 명확하게 알 수 있는 코드에 사용하는 편이 좋다. 추후 코드 변경으로 예외 처리가 더 적절한 경우로 바뀔 수도 있는데 assert 그대로 사용할 수 있기 때문이다.
- 코드상에서 반복적으로 확인될 수 있는 조건이라면 맥락상 예외처리를 하는 부분에서는 조건을 예외로 두어도 좋다. 그런데 맥락상 예외처리를 하지 않는 부분에서는 굳이 예외를 만들어 처리할 필요가 없기 때문에 assert로 두는 것이 나을 수도 있다.
