...
- 다양한 케이스에 대해서 검증을 할 수 있는 테스트 코드가 있다면 Assert는 버그의 원인을 찾는데 도움이 될 수 있다. (테스트 코드에서 사용하는 Assert와 현재 말하고 있는 Assert는 약간 다르다. 로직의 인풋과 아웃풋을 검증하는 테스트 코드와 달리 그냥 Assert는 로직 내부의 코드가 실행 될 때의 값을 검증하는 것이다.)
