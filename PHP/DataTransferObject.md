...
- 연관 배열을 통한 데이터 전달은 관심사를 한번에 모을 수 있고 연관 배열이 너무 유연하기 때문에 사용하기는 쉽지만 플로우의 전후 데이터 구조가 정해져 있지 않다보니 데이터 구조의 변화가 있는지 확인을 해야하는 불편함이 생긴다. 또한 연관 배열 내부에는 타입을 지정할 수 없기 때문에 값을 이용할 수 있는지 없는지 
...
- 이렇게 nested class를 만들어서 사용하게 되면, 이는 이 클래스 내부에서 통제되는 용도로 주로 사용한다.
```
class User
{
    public $money;
