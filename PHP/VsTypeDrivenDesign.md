...
- 구조체와 클래스 (곱 타입) : 클래스의 프로퍼티는 여러 타입으로 구성이 된다. 복수의 타입 값의 상태로 하나의 타입의 값을 정하기 때문에 곱 타입이 된다.
...
프로그래밍에서 복잡성이란, 여러 케이스를 처리해야 하거나, 로직의 뜻이 일반적인 상식으로는 이해하기 어려운 경우일 것이다. 어려운 수학 공식을 사용한다거나, 한 부분의 로직이 여려 조건에서 동작을 할 때 일반적인 지식으로는 생각하기 어려운 경우 복잡하다고 한다.

유니온이나 제네릭의 경우 복잡성을 늘리는데 그 이유는, 유니온의 경우, 각각의 타입에 대한 분기 처리를 해야 하며, 제네릭의 경우 작은 단위의 함수를 조합하는 프로그래밍이 아니라면, 지정한 제네릭이 내부 처리 로직의 어떤 부분에 타입을 적용했는지 알아야 하기 때문에 이러한 변화와 관계성을 쉽게 이해할 수 있는 시그니처를 가지거나 예제가 있는 편이 좋다.

### 타입 어설션

어떤 함수의 반환값에 어떤 타입 A와 null이 포함되어 있다면, 이 코드에 연결되는 코드를 쓸 때, null에 대한 처리도 해 주어야 한다. 그런데 어떤 특정한 파라메터의 범위에 대해서는 함수의 반환 타입이 `null|A`가 아니라 A만 된다고 하자. 이를 명확하게 알 수 있을 때, null에 대한 처리를 뒤에 이을 필요가 없다. 이 때는 `assert`로 반환 타입이 A라는 것을 알려 주면, phpstorm이나 IDE에서는 `assert`로 타입 제한이 들어간 코드 다음으로는 null은 고려하지 않고 A 타입이라고 인식하게 된다. 이러한 방식을 통해서 불필요한 타입 처리를 줄일 수 있다.

하지만 이는 파라메터에 전달되는 타입에 의해서 특정 반환되는 타입이 결정되는 것이 아니라, 값에 의해서 반환 타입의 범위가 결정되기 때문에 런타임에만 확인이 가능하다. 실제 사용되지 않는 타입을 사용하지 않는 것을 통해서 타입으로 인한 분기를 줄일 수 있지만, 런타임의 값의 변화를 확인해야 하는 등의 문제가 있다. 런타임에 의존하는 코드를 추가하기 보다는 런타임에 의존하지 않는 정적 분석이나 컴파일러에 의존하도록 런타임 어설션을 제외하고 유니온 타입 모두에 대해 분기 처리를 하는 방법이 때로는 선호 될 수도 있다.

정적 분석을 통한 타입 확인의 이점을 포기하고, null이 반환되는 엣지 케이스가 제외된다는 것을 런타임으로 확인해야 한다는 것은, 코드가 늘어나는 분기의 복잡성을 줄이지만, 타입 내에서의 값의 변화에 의존하기 때문에 값의 변화에 따른 타입 변화를 주의 깊게 살펴야 한다. 이를 위해서는 로직의 깔끔함이나 명확성 또는 테스트 할 수 있는 코드를 만드는 등의 노력이 필요하다. 런타임에서 동작을 확인하기 쉬운 경우 쓸모없는 코드를 줄여서 로직의 이해를 더 쉽게 하는 편이 선호될 수 있고, 타입에 중점을 둔 언어의 경우 코드가 조금 더 장항해지더라도 에러가 발생하지 않는 코드를 작성하는 쪽이 선호될 수 있다.

인터프리터 언어의 경우, 런타임 확인이 컴파일 언어보다 수월하기 때문에 로직의 복잡성을 줄이는 것이 더 나을 수도 있다. 하지만, 런타임 확인이 간단하기 위해서는 코드의 일부만을 따로 분리해서 동작을 확인하기 쉬운 코드를 만들어야 한다. 하지만, 프레임워크, 전역 변수나 함수에 의존적인 코드를 만드는 경우가 많기 때문에 런타임 확인을 할 수 있는 코드를 만들 수 있는 역량을 키우는 것이 필요하다. 컴파일 언어가 빌드 시간을 단축하기 위해서 유닛 테스트가 가능한 코드를 만드는 쪽을 택하는 반면, 인터프리터 언어는 타입 활용이 적기에 런타임 검사가 중요하므로 런타임 확인을 쉽게 할 수 있도록 코드를 작성해야 한다.
