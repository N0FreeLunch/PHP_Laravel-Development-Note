...
## Closure은 callable 타입의 서브 타입이 아니다.

php에서 호출 가능한 대상은 익명함수, 언어의 내장 함수, 메소드, 기명함수, 클래스에 `__invoke` 메소드가 정의된 객체 등이 php에서 `callalbe` 타입으로 분류된다. 그 중 익명함수는 문법상 클래스 관련 키워드를 직접 사용하지는 않지만, `Closure`라는 클래스를 주형으로 생성되는 객체이다. 익명함수는 `callalbe` 타입이면서 익명함수(`Closure`)도 타입이므로 `Closure`가 `callalbe`의 서브타입이겠지라고 생각하는 경우도 있다. 하지만, 클로저는 `callalbe` 타입의 서브타입이 아니다.

php에서 원시 타입이 아닌 타입은 클래스, 추상 클래스, 인터페이스 등의 심볼이라 불리는 대상을 통해서만 정의할 수 있다. callalbe은 주형이 되는 심볼이 존재하지 않으며, 가상의 심볼이 있다고 하더라도 심볼들 간의 상위-하위 유형의 관계를 가지기 위해서는 메소드명이 일치하는 메소드의 시그니처가 상위 메소드의 시그니처의 확장 형태 (호환이 되면서 추가적으로 무언가를 할 수 있는 형태의 시그니처) callalbe의 인터페이스로 할 수 있는 것을 Closure의 인터페이스가 할 수 없다면 안 된다.

이에 관한 논의는 [다음](https://externals.io/message/125943)을 참고하자.
