...
제네릭을 활용하면 어떤 함수나 클래스가 어떤 타입에 대해서 성립하는지 알 수 있는 정보를 제공할 수 있다. 하지만 제네릭이 없기 어떤 기능을 사용했을 때 얻는 값의 유형에 대한 정보를 얻을 수 없고 이 때문에 런타임 타입검사 또는 런타임 타입힌트를 통해서 유형 정보를 확인할 수 밖에 없다. 강타입 언어 보다 적은 제약의 빠른 생산성을 위해 동적 언어를 사용하는데 런타임의 타입 체크를 위해서 로직을 추가적인 로직을 작성해 줘야 한다면 동적 언어를 쓰는 이점이 있을까?를 생각해 봐야 한다.
