...
무한한 결과값을 가지는 경우에는 배열과 같은 유한한 저장 공간을 사용하는 방식을 사용할 수는 없다. 이 때는, 필요한 때 필요한 만큼만 실행하는 코드를 만들어야 하는데 이런 동작을 만들 때 제너레이터를 사용하면 간단히 만들 수 있다는 장점이 있다.

### 지연 평가 이용하기

제너레이터는 다음 단계로 진행하는 특수한 문법(foreach의 loop와 같은 것)이나 메소드(next)를 사용하므로 이들을 사용하지 않는다면 제너레이터는 다음 단계의 `yield`로 진행하지 않는다. 이런 특성은 다음 단계로 진행시켰을 때만 코드를 실행하기 때문에, 긴 흐름의 로직을 구성할 때, 코드가 전부 실행되지 않고 도중에 처리가 멈췄을 때, 앞선 로직에서 당장 필요하지 않아서 실행하지 않는 코드를 만들기 때문에 시스템 리소스를 절감하는 코드를 작성할 수 있다.
...
함수형 프로그래밍에서 지연 평가는 함수의 실행 지점을 원본 함수의 모든 인자가 채워졌을 때의 시점에서 실행하는 것으로 제너레이터가 각 단계의 실행을 조금씩 지연하여 실행하는 것과는 차이가 있다.
