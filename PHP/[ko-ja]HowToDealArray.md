...
もちろん、コードが適切に作成されていれば、動作を確認しなくても問題ないかもしれません。しかし、信頼しにくいコードが至る所に存在する可能性もあります。したがって、共通で使用されるコードを直接変更するのではなく、一般的な動作を持つ関数を作成し、それらを組み合わせて使用することで、共通の機能はそのままにしつつ、必要な部分だけ関数を追加して動作を変更できるという利点があります。
