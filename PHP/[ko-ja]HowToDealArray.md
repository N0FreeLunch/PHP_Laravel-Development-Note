...
- 배열 내부의 특정 키의 값이 중요한 것이라면, 파라메터로 특정 키가 포함된 배열을 받지 않고 배열의 키 값 파라메터에 대응되도록 정의하는 것도 처리할 대상을 분명히 하기 때문에 괜찮은 방법이다. 그리고 배열이 아닌 배열의 키를 파라메터로 지정하더라도 php에서는 인자로 배열을 전달 할 수 있다.
