...
익명함수를 만드는 것은 함수 외부의 변수를 캡쳐해서 사용할 수 있는 이점이 있고, 해당 맥락에서만 사용하는 코드를 작성하는데 유용하다. 인스턴스 메소드는 `$this`를 자주 사용하게 되어 객체의 상태 관리가 복잡하게 되고 어려워질 수 있으므로, 정적 메소드를 사용하고, 특별한 상황이 아닌 한 정적 메소드는 정적 멤버의 상태 공유가 없는 순수 함수처럼 정의하여 사용하도록 하자.
