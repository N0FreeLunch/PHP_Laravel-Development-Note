# 클로저를 사용하는 더 나은 방법 (나는 왜 클로저를 좋아하지 않는가?)
# クロージャをより良く使う方法（私がクロージャを好まない理由）
...
익명함수를 만드는 것은 함수 외부의 변수를 캡쳐해서 사용할 수 있는 이점이 있고, 해당 맥락에서만 사용하는 코드를 작성하는데 유용하므로 익명함수를 사용할지 메소드를 사용할지 잘 판단하도록 하자.

無名関数を作成することには、関数の外部にある変数をキャプチャして利用できるという利点があります。また、特定のコンテキストだけで使うコードを記述する際にも便利です。そのため、無名関数を使うべきか、メソッドを使うべきかをよく判断しましょう。
