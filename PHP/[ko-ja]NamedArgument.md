...
PHP に名前付き引数がなかった時代には、シングルアクションクラスを使ったり、引数をパラメータの意味を持つ変数に代入してから渡すなどの方法が用いられていました。しかし、PHPに名前付き引数が導入されてからは、より簡潔で可読性の高い引数の渡し方ができるようになりました。
