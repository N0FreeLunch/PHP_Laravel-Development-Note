...
익명함수는 Closure 클래스의 호출 가능한 메소드를 정의하는 것과 같다. 그래서 익명함수의 함수 블록안에 `$this`는 `Closure`를 생성한 스코프의 `$this`를 가리킨다. 이때 객체를 바인딩하는 것을 통해서 익명 함수 블록 내의 `$this`에 다른 객체를 지정하여 접근할 수 있다.

無名関数は、`Closure`クラスの呼び出し可能なメソッドを定義することと同じです。そのため、無名関数の関数ブロック内での`$this`は`Closure`を生成するスコープの`$this`を指します。このとき、オブジェクトをバインドすることで、無名関数ブロック内の`$this`に他のオブジェクトを指定してアクセスできます。
...
참고로 `callable` 타입힌트에 `Closure` 타입을 전달할 수 있다. 그러나 `Closure`는 `callable`의 서브타입은 아니라고 한다.[^1]

なお、`callable`型ヒントに`Closure`型を渡すことができます。しかし、`Closure`は`callable`のサブタイプではないと言われています。[^1]
...
上記のような`calculate`関数は、関数型プログラミングでは`apply`という名前の関数として実装されます。しかし、関数型プログラミングで使用される関数名を活用して関数を定義できる人も、関数型プログラミングでよく使われる関数名から関数の役割や使い方を覚え出す人も少ないです。そのため、関数のパラメータの仕様をできるだけ明示することが望ましいです。
...
런타임 타입 검사를 위해 `checkClosureParam`라는 파라메터의 타입을 검사하는 기능과 `checkClosureReturn`라는 리턴 타입을 검사하는 기능을 만들었다. 여러 타입의 클로저를 처리할 수 있도록 가변 파라메터로 복수의 타입을 지정할 수 있도록 만들어 주었다. 또한 타입힌트가 없는 경우도 허용할지 말지를 정할 수 있도록 빈 문자열('')로 타입힌트가 없는 경우 통과할 수 있도록 만들었다. 또한 리스코프 치환이 가능하도록 `is_subclass_of`으로 타입 체크를 하였다. 이후 한 번 만든 함수를 여러 파일에서 재사용할 수 있도록 `use function checkClosureParam`, `use function checkClosureReturn`으로 불러와서 클로저의 타입을 확인하자. (use function 키워드를 사용하기 위해서는 composer.json의 autoload 설정을 해 주어야 한다.)

ランタイム型チェックのために、パラメータの型を検証する機能として`checkClosureParam`、戻り値の型を検証する機能として`checkClosureReturn`を作成しました。ユニオン型をチェックできるよう、可変パラメータで複数の型を指定できるようにしています。また、型ヒントがない場合も許容するかどうかを指定できるよう、空文字列（’’）で型ヒントがない場合に通過できるようにしました。さらに、リスコフの置換原則に対応するため、`is_subclass_of`を用いて型チェックを行っています。その後、一度作成した関数は複数のファイルで再利用できるよう、`use function checkClosureParam`、`use function checkClosureReturn`で呼び出し、クロージャの型を確認します。(`use function`キーワードを使用するためには、composer.jsonのautoload設定を行う必要があります。)
...
위 코드의 주의점으로는 실제 런타임에 전달되는 클로저의 파라메터나 반환 타입이 유니온 타입일 경우의 로직은 구현하지 않았다. **클로저의 파라메터 타입과 반환 타입이 싱글 타입인 경우만을 가정**한다. 여기서 여러 타입의 파라메터를 받을 수 있도록 고려한 것은 파라메터 타입과 반환 타입이 싱글 타입인 여러 종류의 시그니처인 클로저를 받을 수 있는 있도록 하기 위함이다. 만약 유니온 타입의 파라메터 타입과 반환 타입을 가진 클로저에 대한 타입 검사 로직을 추가하려고 한다면 '런타임 검사를 통해 확인하려는 유니온 타입을 구성하는 각각의 타입은 전달되는 클로저의 유니온 타입 중에서 적어도 어느 하나의 타입과 일치하거나 또는 서브 타입이어야 한다'라는 조건을 만족하는 로직을 만들면 된다. 그리고 여기서는 여러 시그니처를 가변 파라메터로 검사하는 방식을 사용했지만, 좀 더 엄격하게 하면 여러 시그니처를 허용하는 경우는 `(checkClosureParam('TypeA') && checkClosureParam('TypeB') && checkClosureReturn('TypeC')) || (checkClosureParam('TypeA') && checkClosureParam('TypeD') && checkClosureReturn('TypeE'))` 와 같은 방식으로 조합해서 사용하면 된다.

上記コードの注意点としては、実際のランタイムで渡されるクロージャのパラメータや返却型がユニオン型の場合のロジックは実装していません。**クロージャのパラメータ型と返却型が単一型の場合のみを仮定**しています。ここで複数の型を考慮したのは、パラメータ型と返却型が単一型である複数種類のシグネチャを持つクロージャを受け入れられるようにするためです。もしユニオン型のパラメータ型や返却型を持つクロージャに対する型チェックロジックを追加したい場合は、『ランタイム検査によって確認したいユニオン型のそれぞれの型は渡されるクロージャのユニオン型の中で少なくとも一つの型と一致するか、またはサブタイプである』という条件を満たすロジックを作成することでできます。そしてここでは複数のシグネチャを可変パラメータで検証する方法を用いましたが、もう少し厳密にするなら複数のシグネチャを許可する場合は、`(checkClosureParam(0, 'TypeA') && checkClosureParam(1, 'TypeB') && checkClosureReturn('TypeC')) || (checkClosureParam(0, 'TypeA') && checkClosureParam(1, 'TypeD') && checkClosureReturn('TypeE'))`のように組み合わせて使用すればよいです。
