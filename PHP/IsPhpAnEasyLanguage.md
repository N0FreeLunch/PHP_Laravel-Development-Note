...
- 좋은 비즈니스 모델이 있는 회사라면 실제 코드를 짜는 것 이상으로 좋은 설계에 대한 고민이 필요하고 설계와 방향성에 대한 고민을 하는데 시간을 투자하므로 언어가 가져다 주는 좀 더 빠른 생산성이 엄청 큰 역할을 하기는 어렵다고 볼 수 있다. 하지만 때로는 코드를 찍어내어야 할 작업이 있을 수도 있고, 설계 보다는 구현이 더 많은 작업도 있을 수 있다. 이런 점에서 확실이 유리한 점이 있긴하다.
