...
- PHP는 웹 프로그래밍을 C로 짜던 시절에 C의 각종 기능들을 레핑해서 웹을 만드는데 편하게 하기 위한 도구 모음으로 시작한 것이라고 할 수 있다. 언어적인 특정을 점점 더 갖춰가기 시작했지만, 잘 정의된 언어라기 보다는 언어적인 특성을 가진 웹 프레임워크라고 보는 편이 적절하다.
...
### 관습에 의한 코딩의 문제
- php는 상당하 자유로운 언어 중 하나로 꼽힌다. 각종 변수에 타입이 다른 값을 넣을 수 있다거나 자유로운 형 변환이 이뤄지는 등의 여러 동적인 값의 변환을 제공한다. 또한 php의 코드가 실행되는 어느 지점에서라도 브라우저에 화면을 데이터를 전송할 수 있다.
...
- 어떤 언어와 툴 그리고 프레임워크로 시작하느냐에 따라 개발자들의 사고와 가치관의 방향성이 달라진다. 언어나 프레임워크가 주는 제한 속에서 프로그래밍을 하다보면 왜 이렇게 짜야 하는지에 대한 고민을 하게 되고 그 이점을 알 수 있게 되며, 언어나 프레임워크를 벗어나더라도 잘 정립된 방식을 기준으로 생각할 수 있기 때문에 좋은 코드를 작성할 가능성이 높아진다. 이것이 개발자가 생산해 내는 코드의 퀄리티 차이를 만들어 낸다. php 언어에서는 이러한 방향성에 제한을 가하는 문법이 다른 엄격한 언어들에 비해 적으며 프레임워크도 뭔가 엄격하지 못한 문법을 가지고 있기 때문에 다양한 스타일의 코딩이 가능하다.
- 결국 php 언어나 php의 프레임워크 라이브러리가 제공하는 코딩스타일은 좋은 코딩의 방향성을 제한하지 않는 경우가 많기 때문에 좋은 스타일에 대한 코딩 방식을 별도로 학습을 할 필요가 있다. 이런 스타일은 개발자들의 논의나 다른 언어들을 비교하고 다양한 코딩 스타일의 케이스를 비교해가면서 이해할 수 있는데, 충분한 경험이 없는 사람들은 언어의 사용방법이나 구현에 초점을 맞추기 때문에 어떤 방향이 올바른지 판단하는 시간을 거의 갖지 못한다. 또한 많은 경우 php를 사용하는 곳은 영세한 기업이 많은데 체계가 잡혀있지 않은 기업에서 일하게 되면 과도한 야근과 프로그래밍 이외의 각종 도메인에 대한 이해를 엔지니어 쪽에서 직접 해야 하기 때문에 각종 커뮤니티 및 논의들을 충분하게 살피고 검토할 시간을 갖지 못하며 스스로 코딩을 하다가 이렇게 쓰는 거지 않겠어?라는 엉터리 관점이 늘 수 있다.
...
- 웹에서 다루는 여러가지 기능을 C언어로 만들어진 php 엔진에서 지원하기 때문에 빠른 연산을 할 수 있는 기능들을 제공한다. 예를 들어 json 관련 기능들 json_encode, json_decode, json_validate을 지원하는 내장함수가 있으므로 이들 연산은 C 급의 빠른 처리를 통해 해결 가능하다는 장점이 있다.
...
- 좋은 비즈니스 모델이 있는 회사라면 실제 코드를 짜는 것 이상으로 좋은 설계에 대한 고민이 필요하고 설계와 방향성에 대한 고민을 하는데 시간을 투자하므로 언어가 가져다 주는 좀 더 빠른 생산성이 엄청 큰 역할을 하기는 어렵다고 볼 수 있다.
