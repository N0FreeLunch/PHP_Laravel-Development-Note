...
ファイルシステムではディスクのデータをキャッシュするバッファがありますが、php言語ではファイルを読み書きする際に別途のphp自体のファイルバッファが存在します。phpのファイルバッファを使用することで、ファイルシステムのキャッシュからデータを直接取得するよりも、高速なファイル処理が可能になります。phpで`fread`や`fwrite`を使用する際、もしphpのファイルバッファにデータがなければ、phpはファイルシステムから直接データを取得するか、またはファイルシステムにデータを渡さなければなりません。この際、ファイルシステムとphp間でデータ転送が行われ、そのパフォーマンスの低下や遅延が発生します。さらに、同一ファイルに複数のphpプロセスがアクセスする場合、ファイルシステムとphp間でデータの受け渡しが完了するまでの間、相互の衝突を防ぐために一時的に他のファイルハンドラのアクセスが遮断して遅延を発生します。これを解決するために、phpのファイルバッファを使用してデータをキャッシュする方法を使用します。

디폴트로 php에서는 자동으로 파일 버퍼의 사이즈를 설정한다. php에는 파일 쓰기를 위한 버퍼 사이즈를 설정하는 [`stream_set_write_buffer`](https://www.php.net/manual/en/function.stream-set-write-buffer.php) 함수와 읽기를 위한 [`stream_set_read_buffer`](https://www.php.net/manual/en/function.stream-set-read-buffer.php) 함수가 있다. 이 때, 버퍼 사이즈를 0으로 설정하면, php 언어의 버퍼에 파일 데이터가 캐시되지 않는다. 이 경우 여러 파일 핸들러가 동일한 파일에 접근할 때 파일 시스템과 php 간의 데이터 전송이 완료될 때까지 다른 핸들러의 접근을 차단하한다. 동시적 접근이 없어 파일 시스템과 php 간의 작업이 순차적으로 처리되므로 파일의 무결성이 보장되지만, 이로 인해 속도 저하가 발생할 수 있다. php 버퍼 사이즈를 설정하여 데이터를 캐시하는 방식은 파일 시스템과 php 간의 동기화에 필요한 지연을 줄이므로 처리 속도를 조금 더 증가시키므로 동일 파일에 대한 동시적인 엑세스가 없어 무결성 문제가 발생하지 않는 경우에는 php의 파일 버퍼를 적절히 설정하여 캐시 사용을 고려하는 것이 좋다.

デフォルトでphpでは自動的にファイルバッファのサイズを設定します。phpには、ファイル書き込みのバッファサイズを設定する[`stream_set_write_buffer`](https://www.php.net/manual/ja/function.stream-set-write-buffer.php)関数と、読み込みのための[`stream_set_read_buffer`](https://www.php.net/manual/ja/function.stream-set-read-buffer.php)関数があります。この際、バッファサイズを0に設定すると、phpのバッファにファイルデータがキャッシュされません。この場合、複数のファイルハンドラが同じファイルにアクセスすると、ファイルシステムとphp間のデータ転送が完了するまで他のハンドラのアクセスが遮断されます。並行アクセスがないため、ファイルシステムとphp間の処理は順次実行され、ファイルの整合性が保証されますが、その代わりに速度低下が発生する可能性があります。一方で、phpバッファサイズを適切に設定してデータをキャッシュする方法は、ファイルシステムとphp間の同期に要する遅延を削減し、処理速度を向上させます。そのため、同一ファイルへの同時アクセスがなく、整合性に問題が生じない場合には、phpのファイルバッファを適切に設定してキャッシュを活用することを検討するのが良いでしょう。
