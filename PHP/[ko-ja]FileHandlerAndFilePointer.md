...
예를 들어, 4096 바이트의 데이터가 파일 시스템 캐시에 저장하면, 이후 1000, 1000, 1000, 1000, 96으로 나눠 5번 접근하는 것보다 1024로 4번 캐시 데이터를 가져오는 것이 캐시 효율을 높인다. 또한 파일 시스템이 여러 블록의 데이터를 캐시할 수 있으므로, 경우에 따라 8192 바이트와 같은 더 큰 단위로 읽는 것도 효율적이다. 이는 메모리 사용량을 최소화하기 위한 2000년대 이전의 코딩에서는 중요했지만 현재로는 크게 중요한 부분은 아니라서 굳이 이를 고집할 필요는 없을 것으로 보인다.

例えば、4096 バイトのデータがファイル システム キャッシュに保存されると、この後、1000バイトずつ4回と96バイトを追加でアクセスするよりも、1024バイトずつ4回キャッシュからデータを取得する方がキャッシュ効率が向上します。また、ファイルシステムが複数のブロックをキャッシュできるため、場合によっては8192バイトのようなより大きな単位で読み取ることも効率的です。これは、メモリ使用量を最小限に抑えることが重要だった2000年代以前のコーディングでは意義がありましたが、現在ではそれほど重要ではなく、無理にこだわる必要はないと思われます。

#### php 언어에서 파일 데이터를 캐시하기
#### php言語でのファイルデータをキャッシュ

파일 시스템에서 디스크의 데이터를 캐시하는 버퍼도 있지만, php 언어에서 파일을 읽거나 쓸 때 별도로 php 자체의 파일 버퍼가 존재한다. php 언어 자체의 파일 버퍼를 사용하면 파일 시스템의 캐시에서 데이터를 직접 가져오는 것보다 더 빠른 파일 처리 속도를 얻을 수 있다. php에서 fread나 fwrite를 사용할 때, 만약 php의 파일 버퍼에 데이터가 없다면, php는 파일 시스템에서 직접 데이터를 가져오거나 파일 시스템에 데이터를 전달해야 한다. 이 과정에서 파일 시스템과 php 간의 데이터 전송이 필요하고, 이로 인해 성능 저하나 지연이 발생하는 경우가 있다. 또한, 동일한 파일에 대해 여러 php 프로세스가 접근하는 경우, 파일 시스템과 php 간의 데이터 전달이 완료될 때까지 서로 간의 충돌 방지를 위해 잠시 다른 파일 헨들러의 접근을 차단하여 지연이 발생한다. 이를 해결하기 위해, php의 파일 버퍼에 데이터를 캐시하는 방식을 사용한다.

ファイルシステムではディスクのデータをキャッシュするバッファがありますが、php言語ではファイルを読み書きする際に別途のphp自体のファイルバッファが存在します。phpのファイルバッファを使用することで、ファイルシステムのキャッシュからデータを直接取得するよりも、より高速なファイル処理速度を得ることができます。phpで`fread`や`fwrite`を使用する際、もしphpのファイルバッファにデータがなければ、phpはファイルシステムから直接データを取得するか、ファイルシステムにデータを渡さなければなりません。この際、ファイルシステムとphp間でデータ転送が行われ、その結果として性能の低下や遅延が発生する場合があります。また、同一ファイルに複数のphpプロセスがアクセスする場合、ファイルシステムとphp間でデータ転送が完了するまで、互いの衝突を防ぐために一時的に他のファイルハンドラのアクセスがブロックされ、遅延が発生します。これを解決するために、phpのファイルバッファにデータをキャッシュする方法を使用します。

디폴트로 php에서는 자동으로 파일 버퍼의 사이즈를 설정한다. php에는 파일 쓰기를 위한 버퍼 사이즈를 설정하는 [`stream_set_write_buffer`](https://www.php.net/manual/en/function.stream-set-write-buffer.php) 함수와 읽기를 위한 [`stream_set_read_buffer`](https://www.php.net/manual/en/function.stream-set-read-buffer.php) 함수가 있다. 이 때, 버퍼 사이즈를 0으로 설정하면, 여러 파일 핸들러가 동일한 파일에 접근할 때 파일 시스템과 php 간의 데이터 전송이 완료될 때까지 다른 핸들러의 접근을 차단하한다. 파일 시스템과 php 간의 작업이 동기화되며, 파일의 무결성이 보장되지만, 이로 인해 속도 저하가 발생할 수 있다. 파일의 무결성을 보장하는 것이 중요한 경우, php 버퍼 사이즈를 설정하여 데이터를 캐시하는 방식은 유효한 방법이지만, 속도가 중요한 작업에서는 주의가 필요하다. 무결성이 조금 손상되어도 괜찮은 경우에는 php의 파일 버퍼를 적절히 설정하여 캐시 사용을 고려하는 것이 좋다.

デフォルトでphpではファイルバッファのサイズを自動的に設定します。phpには、ファイル書き込みのバッファサイズを設定する[`stream_set_write_buffer`](https://www.php.net/manual/ja/function.stream-set-write-buffer.php)関数と、読み込みのための[`stream_set_read_buffer`](https://www.php.net/manual/ja/function.stream-set-read-buffer.php)関数があります。この時、バッファサイズを0に設定すると、複数のファイルハンドラが同一のファイルにアクセスする際、ファイルシステムとphp間でデータ転送が完了するまで他のハンドラーのアクセスをブロックします。ファイルシステムとphp間の作業が同期され、ファイルの整合性が保証されますが、その結果として速度低下が発生することがあります。ファイルの整合性を保証することが重要な場合、phpのバッファサイズを設定してデータをキャッシュする方法は有効な手段ですが、速度が重要な作業では注意が必要です。整合性が多少損なわれても問題ない場合には、phpのファイルバッファを適切に設定してキャッシュを利用することを検討するのが良いです。
