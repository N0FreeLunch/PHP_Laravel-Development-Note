...
ファイルロックを解除するために使用します。ロックをかけたハンドラのみが、かけられたロックを解除することができます。
...
파일을 열고 파일 헨들러를 반환 받는다. 이 파일 헨들러는 stream이라고 부르며 파일 시스템 포인터를 다룰 수 있는 리소스를 반환한다.

ファイルを開き、ファイルハンドラを返します。このファイルハンドラはストリームと呼ばれ、ファイルシステムポインタを扱えるリソースを返します。
