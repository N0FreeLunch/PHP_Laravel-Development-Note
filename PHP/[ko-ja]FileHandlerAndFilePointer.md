...
コンピュータがハードウェアに記録されたデータを読み書きする場合、ブロックという単位でデータを読み書きします。ブロックとは、データを一度に扱うためのデータ単位である。ディスクでデータを読み取る際は、どんなに小さなデータを読み書きする場合でも、1単位のブロックを読み取らければなりません。ディスクへのアクセスはファイルシステムで管理され、ファイルシステムは1つのブロックのデータを読み込むために、少なくともブロックのサイズ以上のメモリ空間を使用する必要があります。ブロックはデータの読み書きの最小単位の概念です。
