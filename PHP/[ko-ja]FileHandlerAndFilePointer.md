...
파일 헨들러를 닫는다. 파일 헨들러를 닫게 되면 OS가 관리하고 있는 파일락이 해제 되면서 이 헨들러가 아닌 다른 작업으로 파일 변경을 할 수 있게 만들어 준다.

ファイルハンドラを閉じます。ファイルハンドラを閉じると、OSが管理しているファイルロックが解除され、他の操作によるファイル変更が可能になります。
