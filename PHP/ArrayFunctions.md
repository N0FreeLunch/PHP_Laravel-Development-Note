...
- callable에 대한 대안으로 오브젝트가 아닌 타입에서 오브젝트처럼 기본 메소드를 제공하는 방식. 스칼라 형태의 타입에서 오브젝트 처럼 `->`으로 해당 타입의 값과 관련된 기본 메소드를 제공하는 방향으로의 모색을 고려하고 있다. 하지만 스칼라 오브젝트에 대한 활발한 논의가 이뤄지지 않고 한동안은 callable 위주의 코딩 방식을 고수할 것으로 보인다.
...
- 라라벨 프레임워크 내에서는 helper 함수로 배열을 컬렉션으로 바꿀 수 있는 `collect(배열)->메소드()`의 방식을 제공한다.
```php
