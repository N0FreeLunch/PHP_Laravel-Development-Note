...
변수에 값을 재할당 하는 방식으로 사용해야 한다. 뒤에서도 살펴 보겠지만 php에서는 이런 방식의 사용이 배열 함수를 사용함에 있어서 가장 깔끔한 코드를 만든다.
...
`|>`의 대상이 되는 함수는 배열이라는 인자를 하나 받는 함수이어야 한다. 

위의 예제에서 `array_filter`, `array_map`은 인자를 두 개 이상 받는 함수이기 때문에 인자를 하나 받는 함수로 만들어 주기 위해 `fn($x) => ...`의 방식으로 코드를 감싸 주었다.
...
php는 네이티브 컬렉션 함수가 존재하지 않는다. 자바스크립트의 경우 배열은 객체이고 `array.method1().method2().method3()`의 방식으로 체이닝을 할 수 있다. 하지만, php의 경우 배열은 원시 타입으로 객체가 아니다. 따라서 배열의 메소드가 존재하지 않는데, 이를 체이닝하기 위한 방법으로 파이프 연산자를 추가한 것이다. 앞선 메소드 체이닝의 표기가 `array |> function1() |> function2() |> function3()`으로 바뀐 것이다.

물론 메소드 체이닝의 경우 `.`을 입력할 때 사용할 수 있는 메소드의 목록이 나오는 반면, `|>` 를 사용할 때는 IDE의 제안이 나올지는 모호한 부분이 있고 추후 IDE의 지원을 확인해 봐야 할 것이다.

또한 이후 플레이스 홀더 ?를 사용한 커링된 함수를 반환하는 방식의 모색에 대한 논의도 이뤄지고 있기 때문에 함수형 프로그래밍의 활용 방안을 모색해 볼 수도 있다.[^2]
...
지연평가를 사용하기 위해서는 커링 가능한 함수를 만들어야 한다. 왜냐하면 일부 인자를 넣고 인자가 다 찰때까지 실행되는 것이 아니라 함수 자체로 존재해야 하기 때문이다. 하지만 php의 언어 내장 함수의 경우, 변경 시도 자체가 불가능하기 때문에 커링을 사용할 수 없으며 지연평가를 위한 커링 함수를 만들기 위해서는 기본 배열함수를 변경 하는 것이 아닌 레핑을 해서 새로운 클로저를 만들어야 한다.

php8.1 부터는 [일급 콜러블 문법](https://www.php.net/manual/en/functions.first_class_callable_syntax.php)을 제공하기 때문에 `$closure = internal_function(...)`으로 클로저를 만들 수 있어 좀 더 지연평가를 도입하기 쉬워진 장점이 있다.
...
라라벨 컬렉션 라이브러리는 라라벨 버전 8 버전 부터 별도 설치가 가능해졌기 때문에 라라벨 버전 8이 요구하는 php 버전이 요구된다.[^3] 그 이전의 버전에서 컬렉션 기능을 사용하고 싶은 경우에는 [collect](https://github.com/tighten/collect) 라이브러리를 사용하도록 한다.
