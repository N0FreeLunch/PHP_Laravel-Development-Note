...
- `array_waik`함수는 첫 번째 인자로 배열을 받고, 두 번째 인자로 콜백함수를 받는다. `array_map`과 달리 단일 배열의 원소를 순회하여 콜백함수를 적용하는 형식이지만, 기존의 배열함수들과 같이 첫 번째 인자로 배열을 받으므로 좀 더 일관성 있는 `map` 함수에 해당한다. 일반적으로 `map` 함수를 사용할 일이 있다면 `array_waik` 함수를 사용하도록 하자.
