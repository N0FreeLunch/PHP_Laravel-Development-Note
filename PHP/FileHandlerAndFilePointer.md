...
데이터는 여러 블록에 분산 저장이 되므로 1024 바이트의 데이터만을 읽는다고 해서 하나의 블록 내의 데이터를 읽는다는 보장은 없다. 하지만 컴퓨터는 파일의 변경이 아닌 생성인 경우, 저장 공간에 여유가 있다면 가능한 연속적인 데이터 저장을 하기 때문에 블록 단위(4096 바이트)의 약수나 배수의 값으로 읽는 것이 좀 더 효율이 좋을 수 있다. 예를 들어 블록 단위로 데이터를 읽기 때문에 OS나 하드웨어 저장소에서 4096의 데이터를 읽어서 캐시에 저장을 하고 있다면 1000, 1000, 1000, 1000, 96으로 5번 데이터를 가져오는 것 보다는 1024로 4번 캐시에서 가져다가 쓰는 것이 효율이 좋다.
