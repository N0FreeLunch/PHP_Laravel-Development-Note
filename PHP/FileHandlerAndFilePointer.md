...
파일 스트림에 관한 예제를 보면 파일 데이터를 읽을 때 1024, 2048, 4096, 8192의 단위를 자주 사용한다는 것을 볼 수 있다. 하드웨어의 데이터를 읽을 때는 블록 단위로 읽는데 하나의 블록은 보통 4096 바이트의 크기를 갖는다. 하드웨어는 4096 보다 더 적은 데이터를 읽어도 블록 단위인 4096 만큼의 데이터를 최소한 읽는다. OS의 파일 시스템은 한 번 읽은 데이터를 캐시해 두기 때문에 데이터를 블록 데이터의 크기 보다 작은 데이터를 취득하더라도 파일 시스템에 캐시된 데이터에 접근하기 때문에 큰 속도 저하는 없다.

데이터는 여러 블록에 분산 저장이 되므로 1024 바이트의 데이터만을 읽는다고 해서 하나의 블록 내의 데이터를 읽는다는 보장은 없다. 하지만 컴퓨터는 파일의 변경이 아닌 생성인 경우, 저장 공간에 여유가 있다면 가능한 연속적인 데이터 저장을 하기 때문에 블록 단위(4096 바이트)의 약수나 배수의 값으로 읽는 것이 좀 더 효율이 좋을 수 있다. 예를 들어 블록 단위로 데이터를 읽기 때문에 OS나 하드웨어 저장소에서 4096의 데이터를 읽어서 캐시에 저장을 하고 있다면 1000, 1000, 1000, 1000, 96으로 5번 데이터를 가져오는 것 보다는 1024로 4번 캐시에서 가져다가 쓰는 것이 효율이 좋다. OS에서는 여러 블록의 데이터를 캐시하는 경우가 있을 수 있기 때문에 4096 바이트 보다 큰 8192의 단위도 때때로 유효하다.

서버의 하드디스크 조각 모음을 주기적으로 해 주던 시기에서는 데이터가 연속적으로 저장이 되어 있어서 4096의 약수인 512, 1024, 2048 등으로 나눠서 읽는 것이 속도 향상에 중요한 역할을 했지만, 조각 모음을 하지 않았거나 SSD를 사용하여 데이터의 조각화가 속도에 영향을 주지 않는 경우에는 블록 단위의 약수나 배수를 고집할 필요는 없다.
