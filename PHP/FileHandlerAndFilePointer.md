...
서버에서 하드디스크 조각 모음을 주기적으로 하던 시기에는, 데이터를 연속적으로 저장할 수 있었기 때문에 4096 바이트 블록의 약수인 512, 1024, 2048 바이트 단위로 나눠서 읽는 것이 속도 향상에 도움이 됐다. 그러나 조각 모음을 하지 않거나 SSD를 사용하는 경우에는, 데이터 조각화가 성능에 미치는 영향이 적으므로 블록 단위의 약수나 배수 단위로 읽는 것이 크게 효율적이지 않아, 이를 고집할 필요는 없다.

サーバーでハードディスクのデフラグを定期的に行っていた時期には、データを連続的に保存できたため、4096バイトブロックの約数である512、1024、2048バイト単位に分けて読み取ることで速度向上に寄与していました。しかし、デフラグを行わない場合やSSDを使用する場合は、データの断片化が性能に与える影響が少ないため、ブロック単位の約数や倍数単位で読み取ることにこだわる必要はありません。
