...
파일을 조작할 때 기본적으로는 자동적으로 파일 락이 걸린다. `fopen` 함수를 사용해서 파일 모드에 `n` 플레그를 추가하면 락을 걸지 않고 조작할 수 있는데 `fopen('faile/path.ext', 'rn')`, `fopen('faile/path.ext', 'wn')`, `fopen('faile/path.ext', 'an')`을 사용하면 락을 걸지 않고 파일을 조작할 수 있다. 그러나 이런 방법은 외부 헨들러로 인해 파일 포인터의 상대적 위치가 달라질 수 있기 때문에 추천하지 않는 방법이다.
