...
파일 스트림에 관한 예제를 보면 파일 데이터를 읽을 때 1024, 2048, 4096, 8192의 단위를 자주 사용한다는 것을 볼 수 있다. 하드웨어의 데이터를 읽을 때는 블록 단위로 읽는데 하나의 블록은 보통 4096 바이트의 크기를 갖는다. 4096 보다 더 적은 데이터를 읽어도 4096 만큼의 데이터를 최소한 읽어야 하므로 너무 작은 비트를 읽는 것은 하드웨어 저장소에 대한 엑세스를 늘리는 것이므로 속도의 측면을 고려했을 때, 한 번 데이터를 취득할 때 일정 데이터 이상을 취득하는 것이 좋다.
