...
파일 헨들러를 통해서 파일을 조작할 때 하나의 파일을 여러 파일 헨들러가 조작을 하는 경우 파일의 파일의 시작점에서 동일한 인덱스의 위치라고 하더라도 데이터 간의 상대적인 위치는 달라질 수 있다. 그래서 파일의 데이터를 변경할 때는 다른 헨들러의 파일 포인터가 의도와는 다른 데이터를 건드리는 경우가 생기므로 읽거나 쓸 수 없도록 락을 건다.

쓰기 모드(w)로 파일을 헨들러가 파일을 점유하고 있을 때는 파일 잠금을 걸어 다른 헨들러가 파일을 읽기 모드(r)로 열거나, 쓰기 모드(w)로 덮어 쓰거나, 추가 모드(a)로 추가할 수 없게 만든다.

읽기 모드(r)로 파일을 헨들러가 파일을 점유하고 있을 때는 다른 파일 헨들러가 파일을 읽기 모드로 열 수는 있지만, 쓰기 모드(w)로 덮어쓰거나 추가 모드(a)로 추가할 수 없게 만든다.
