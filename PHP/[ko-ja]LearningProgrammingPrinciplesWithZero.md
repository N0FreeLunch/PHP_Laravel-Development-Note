...
- しかし、初期値を空の値にする場合、nullかどうかをif文で確認する必要がなく、先述の多くの組み込み関数の機能に対して空の値が処理前後の状態が同じなので、未割り当ての場合には実行しない別個のif文を使用する必要もありません。今後のphpの変化に合わせて、初期値としてnullを使用するよりも、基本的に空の値を初期値として使用し、購入価格が0の場合に無料で購入するみたいに、空の値が有効な値になる場合、nullを初期値として使用する方法を採用することで、多くの利点を享受できます。
