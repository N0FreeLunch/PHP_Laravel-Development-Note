...
- 고정값 개념은 0의 성질을 설명하기 위해서 도입한 것이며, `n x 0 = 0`의 경우와 같이 `y = f(x)`에서 x가 프로그래밍 언어에서 변수가 되는 경우도 있지만, `n + 0 = n`의 경우와 같이 `f(n) = n` n이 고정값이 아니라 고정값의 방정식의 꼴을 만들기 위해 0이 쓰이는 경우도 존재하므로 프로그래밍 언어에서 항상 변수가 고정값이 되는 것은 아니라는 점에 유의하자.
- 不動点の概念は、0の性質を説明するために導入されました。`n x 0 = 0`のような場合、`y = f(x)`で「x」がプログラミング言語で変数になる場合もありますが、`n + 0 = n`のような場合や、`f(n) = n` nが不動点ではなく、不動点の方程式の形を作るために0が使われる場合もあるため、プログラミング言語で常に変数が不動点になるわけではないことに注意してください。
