...
### 처리 전후의 상태가 동일함
### 処理の前後の状態が同じ
- 일반적으로 프로그래밍 언어에서 상태라는 것은 값으로 표현된다. 값은 변수에 저장되어 있을 수도 있고, 함수의 반환 값이 될 수도 있고, 스칼라값 그대로 표현되기도 한다. 어떤 처리를 하기 전의 값과 어떤 처리를 한 이후의 값이 동일한 값을 가지고 있다면 이는 동일한 상태를 가진것과 같다.
