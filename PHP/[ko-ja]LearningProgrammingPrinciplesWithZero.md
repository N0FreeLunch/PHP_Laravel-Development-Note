...
- 値または変数「x、y」についてのアルゴリズム「f」があると仮定しましょう。`f(x, y) = x`の関係で、「x」を変数とし、「y」を値としてみましょう。すると、`f'(x) = x`を満たす関数`f'`を作成できます。アルゴリズム`f'`は、アルゴリズムを実行した場合と実行しなかった場合のの結果が同じであるため、ロジック的には実行されないのと同じです。もちろんプログラムは実行されますが、実行前後の状態は同じです。
