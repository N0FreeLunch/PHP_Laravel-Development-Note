...
- 구매하지 않은 상태로 변수가 가질 수 있는 값이 두 가지 0과 null이 될 수 있는 경우, `if(!is_null($var) && $var !== 0)`로 할당된 상태인 경우에만 if문 내부의 코드를 실행하는 조건문의 코드가 늘어나게 되며, 긴 조건문을 사용하고 싶지 않다면 `empty`를 사용하게 되는데 `empty`는 변수의 타입을 드러내지 않기 때문에 개인적으로 선호하지 않는 코딩 스타일이다.
