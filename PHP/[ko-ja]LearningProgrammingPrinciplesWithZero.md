...
- phpでは、変数にさまざまな型の値を入れることができますが、型を多用するほど分岐処理が増えるため、一つの型または一つの型とnullを併用した型を使用することをお勧めします。この条件下で考えてみましょう。変数の初期値として空の値を使用する場合、未割り当ての値の型が割り当てられた型と一致します。たとえば、`$var = 0`を初期値として割り当てたから同じタイプである`$var = 10`という値を割り当てるという事です。nullが初期値の場合とは異なり、空の値は割り当てられた値の型と同じ型です。一方、nullを初期値として使用する場合、割り当てられた値は異なる型です。たとえば、引数のタイプヒントでintを指定した場合、nullを渡すと型不一致のためエラーが発生します。nullを初期値として使用すると、型不一致によりエラーが発生するため、関数やメソッドのパラメーターとして、変数に値が割り当てられていない値を渡すときに、値が割り当てられたかどうかをエラーで確認しやすいという利点があります。
