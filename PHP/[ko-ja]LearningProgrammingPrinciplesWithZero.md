## 0으로 배우는 프로그래밍 생활 (부제: 초기값으로 null인가 빈 값인가?)
## ゼロからまなぶプログラミング生活 (サブタイトル: 初期値はnullか空の値か？)
...
### 빈 값
### 空の値
- 빈 값도 0의 예시 처럼 아무런 영향을 주지 않는 값으로 사용하는 코드를 만들 수 있다. 빈 값에 대한 로직의 처리 전후의 상태의 변경이 없다면, 알고리즘은 실제로 실행되었지만 마치 실행되지 않은 것과 동일한 상태를 갖는다.
- 空の値も、0のように何の影響も与えない値として使用するコードを作成することができます。空の値に対するロジックの処理前後の状態（変数の値）の変更がない場合、アルゴリズムは実際には実行されたものの、まるで実行されなかったかのような状態を持ちます。
- 값 또는 변수 x,y에 대해 알고리즘 f가 있다고 하자. `f(x,y) = x`의 관계에서 x를 변수로 y를 값으로 두자. 그럼 `f'(x) = x`을 만족하는 `f'`라는 함수를 만들 수 있을 것이다. 알고리즘 `f'`는 로직상 실행되지 않는 것과 동일하다. 알고리즘의 실행 전후의 결과가 동일하기 때문이다. 물론 프로그램이 실행되면서 리소스를 소비하지만 전후의 값/변수 상태의 결과값은 동일하다. 값/변수 x,y,z의 초기값이 null일때, if(is_all_null(x,y,z)) { f(x,y,z) }의 코드가 실행이 되더라도 x,y,z가 null이므로 if문을 통과하지 못하여 f(x,y,z)는 실행되지 않는다. 이런 로직과 초기값으로 빈 값을 사용했을 때 if문 없이 f(x,y,z)를 실행했을 때 x,y,z의 값이 빈 값이 되는 것과 처리 전후의 상태는 null에서 null으로 빈값에서 빈값으로 변수 상태는 동일하다.
- 変数 「x、y、z」とアルゴリズム「f」があるとします。アルゴリズム「f」を実行しても変数「x、y、z」が同じ値であれば、「f(x,y,z) = (x,y,z)の関係で、「f」はロジック上、実行されないことと同じです。もちろんプログラムは実行されてリソースを使いますが、前後の変数状態の結果は同じです。変数「x、y、z」の初期値がnullの場合、「if(is_all_null(x,y,z)) { f(x,y,z) }」のコードが実行されても、「x,y,z」がnullであるため、if紋を通過することができなくて「f(x,y,z)」は実行されません。このようなロジックと初期値としての空の値を使用した場合、if文なしに「f(x,y,z)」を実行した場合、x、y、zの値は空の値になり、処理前後の状態はnullからnullで、空の値から空の値で変数状態は同じです。
