...
- `const`: 모르는 새에 변수 섀도잉이 일어나는 일이 없고, 함수 코드의 길이를 짧게 쓰면 굳이 재정의 불가능한 키워드를 사용하지 않아도 되기 때문에 무리하게 `const` 키워드를 도입할 필요가 없다.
- `const`: 意図しない変数のシャドーイングが発生することがなく、関数のコードを短く書けば再定義不可のキーワードを無理に使う必要もないため、constキーワードを無理に導入する必要はありません。
