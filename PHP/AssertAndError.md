...
프로덕션 환경에서 무조건 `assert`를 통과하지 못했을 때 에러가 발생하도록 했다면 일부 케이스에 관해서는 에러가 발생하지 않도록 하고 싶어도 할 수 없기 때문에 프로덕션 환경에서 에러를 발생시키고자 하는 것은 `assert`가 아닌 커스텀 예외로 처리하는 것이 옳다.
...
전역 예외를 사용하는 방법은 `Exception` 클래스를 하위 클래스를 정의하지 않고 그냥 그대로 사용하는 것이다. 전역 에외를 일으키는 `Exception` 클래스의 인스턴스는 `catch` 문에서는 `Exception` 서브 타입을 사용하며 `Exception` 클래스를 그대로 잡지 않는 코딩 방식을 사용하기 때문에 `catch` 문에서 잡지 않는 `Exception` 클래스를 이용해서 처리를 멈추는 스타일의 코딩을 할 수 있다. 
...
## 어플리케이션의 경계에서는 예외를 사용하자.

애플리케이션이 외부 데이터를 가져와서 실행하는 부분에서는 정말 예기치 못한 상황이 있을 수 있고, 이런 로직은 있을 수 없을 텐데... 혹시라도 생긴다면 이라는 부분이 있다. 컨트롤러의 리퀘스트 벨리데이션이나, 디비의 컬럼 값을 캐스팅하는 부분, 임피던스 미스매칭을 방지하기 위한 정확한 매칭 등의 경계에서는 예기치 않은 처리가 일어날 수 있기 때문에 예외로 처리하는 것이 맞다. 모든 케이스를 방어할 수 없거나, 모든 예외 케이스가 어떤 것이 있는지 불명확한 부분이 이에 해당한다.

하지만 애플리케이션의 내부에서는 경계에서 상정하지 않은 부분에 대해서는 에러나 예외가 발생했기 때문에, `assert`로 동작할 조건 정도를 가볍게 유지하여 디버깅이 편한 정도로만 유지하면 된다. 만약, `assert`에서 에러가 발생했다면, 애플리케이션의 경계 부분을 좀 더 다듬으면 되는 것이고, 내부에서는 어떤 기능과 맞물려서 동작하는지에 따라 조건을 넓히거나 좁히는 방향으로 사용하면 된다.
