...
- 0에 어떤 값을 곱했을 때, 결과는 0이 된다.
- 0애 어떤 값을 나누었을 때, 결과는 0이 된다.
- 0에 대한 곲셈과 나눗셈을 생각해 보자. 0에 어떤 값을 연산 했을 때, 어떤 값과 연산은 존재하지 않았던 것과 다름없다.

#### 항등원(identity element)과 자기동형(automorphism)
...
### 행위가 일어나지 않는 의미로서의 자기동형
- 상품을 구매할 수 있는 사이트의 구매 수량이 0인 경우는 구매를 하지 않는 것이 된다. '상품 가격' x '상품 수량' x '수수료 배율 + 세금 배율'이 총 지불할 가격이 될 것이다. 이 때 상품 수량이 0인 경우 어떠한 구매도 일어나지 않는다. 곧 행위가 일어나지 않는 것이다.
- 빈 값으로 향하는 자기동형의 값을 이용하여 행위의 일어나지 않음을 표현할 수 있다.
