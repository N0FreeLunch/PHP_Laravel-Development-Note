...
- 또한 앞서 변수가 정의되지 않은 경우 할당을 위해서 `isset`, `??`, `=??`을 사용하자고 하였다. 이 때 `isset`의 결과가 `false`일 때 변수가 정의되어 있더라도 `null`인 경우만 생각하면 된다. `null`은 보통 각 타입의 빈 값을 비할당을 식별하는 용도로 사용할 수 없는 경우 타입을 추가하여 비할당을 판별하기 위해서 사용한다. `null`이 아닌 각 타입의 빈값을 사용하는 것은 사용되는 타입의 범위를 줄이는 역할을 하는 것으로, `null`이 할당된 변수에 디폴트 값으로 각 타입의 빈값을 할당하여 할당과 비할당을 구분하는 것은 아무런 문제가 되지 않느다. 하지만 `empty`의 경우 통과 될 수 있는 값이 여럿이기 때문에 기존 값을 잘못 덮어 씌울 가능성이 있다. 예를 들어 변수에서 `0`이란 값을 비할당으로 사용하고 있었는데 디폴트 값으로 `null`을 할당하여 빈 값의 케이스를 타입의 빈 값과 `null` 두 가지를 고려해야 하는 것으로 만들어 버릴 수 있다.
- また、先に変数が未定義の場合は、割り当てのために`isset`, `??`, `=??`を使いましょう」と書きました。この場合、`isset`の結果が`false`の場合、変数が定義されていても`null`の場合だけを考えれば十分です。 `null`は通常、各タイプの空の値を非割り当てを識別するために使用され、タイプを追加して非割り当てを判別するために使用します。`null`ではなく、各タイプの空の値を使用することは、使用されるタイプの範囲を減らす役割を果たすもので、`null`が割り当てられた変数にデフォルトの値として各タイプの空の値を割り当て、割り当てと非割り当てを区別することは何の問題にもなりません。しかし、`empty`の場合、通過できる値が多数あるため、既存の値を誤って上書きする可能性があります。 例えば、変数で`0`という値を非割り当てとして使用していましたが、デフォルト値として`null`を割り当て、空の値のケースをタイプの空の値と`null`の2つを考慮すべきものにしてしまうことがあります。
