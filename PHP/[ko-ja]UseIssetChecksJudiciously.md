...
2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てしません。単に割り当てられていないため未定義の変数が伝達される場合、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に応じる非割り当ての値をデフォルト値として代入し、非割り当てが未定義の変数になることを防止します。(前のコードの実行で必ず生成され伝達されるべき値は、未定義になって、ロジックの実行過程でエラーを発生させることが妥当であるため、デフォルト値を割り当てません。これに関しては後の例のコードの必須・オプショナルメンバー部分をご参考ください。)
...
1. 
    1. 変数を宣言する際、変数のデフォルト値を指定します。 変数の値が複数の条件によって変わる場合は、メソッドや関数などの戻り値を使用して変数の値を受け取るようにします。
    2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てしません。単に割り当てられていないため未定義の変数が伝達される場合、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に応じる非割り当ての値をデフォルト値として代入し、非割り当てが未定義の変数になることを防止します。
    3. ロジックを展開するのに必要な変数がすべて集まった後、ロジックの流れを表すコードを作成します。
2. 
...
- 값의 존재 비존재로 할당과 비할당을 구분할 경우 문제가 있는데 php7 부터 타입힌트가 도입이 되었고 대부분의 모던 php의 코드들은 타입힌트를 적극 사용한다. 함수나 메소드에 값을 넘겨 줄 때 타입힌트를 사용하게 되면 해당 값은 모두 정의된 변수로 넘겨 주게 된다. 함수나 메소드 스코프에서 값의 할당/비할당을 확인할 때, 타입힌트로 전달된 값은 할당인지 비할당인지 이미 정의된 값이므로 `isset`을 사용하지 않는 코드를 작성할 수 있다. `isset`을 정의되지 않은 변수를 확인하는 용도로 사용하자라고 했으므로, 타입힌트로 전달된 값에는 `isset`을 사용하지 않는 상황인데, 스코프내에서 새롭게 사용할 변수는 `isset`으로 할당되었는지 확인한다면 일관성 측면에서 좋지 않다. 정의된 값으로 할당 비할당을 확인하는 변수들이 반드시 존재하므로, 일관성을 위해서 정의된 변수로 할당 비할당을 구분하는 방법을 사용하는 편이 좋다.
- 値の存在有無で割り当てと非割り当てを区分する場合問題があります。php7からタイプヒントが導入され、ほとんどのモダンなphpのコードでは積極的にタイプヒントが利用されます。関数やメソッドに値を渡す際にタイプヒントを使用すると、その値はすべて定義された変数に渡されます。関数やメソッドスコープ内で値の割り当て・非割り当てを確認する際に、
タイプヒントとして渡された値は割り当てられているかどうかは、既に定義されているため、`isset`を使用しないコードを作成することができます。`isset`を未定義の変数を確認する用途に使おうとしたので、タイプヒントで伝達された値には`isset`を使用しないスタイルの状況で、スコープ内で新たに使用する変数は`isset`で割り当てられているかを確認すると、一貫性が損なわれます。定義済みの値で割り当て非割り当てを確認する変数が必ず存在するため、一貫性を保つためには、定義された変数で割り当て非割り当てを区分する方法を採用するのが良いと思います。


#### 하나의 타입으로 할당과 비할당을 구분하기
#### 一つのタイプで割り当てと非割り当てを区別する
...
- 1つのタイプだけを使用しても、割り当て・非割り当ての状態を表現できる場合があります。デフォルト値として空の値を利用する方法です。通常、文字列の場合は空の文字列`""`と空でない文字列で、数の場合は`0`と`0`でない数で、配列は空の配列`[]`と空でない配列`[1,2,3]`で、1つのタイプで空の値と空でない値を区分する方式を使用し、デフォルト値で空を割り当てる方式です。

#### null 타입으로 비할당 상태 구분하기
#### nullタイプで非割り当て状態を区分する
...
- 例えば、購入数量というのは、0なら購入していないもので、0でない数なら購入したものです。そのため、1つのタイプで割り当ての状態を区分することができます。しかし、価格は0であることもあります。そのため、価格は1つのタイプで割り当てるかどうかを区分できない特性があります。 価格の場合、非割り当てを示すために数タイプ以外に`null`タイプを追加して使用します。
- 때때로 `null`이란 값이 할당된 값이 될 수도 있다. 이 경우에는 `null`인 변수에 `null`을 다시 할당을 하면 되기 때문에 특별히 문제가 있지 않다.
- 時々`null`という値が割り当てられることがあります。この場合、`null`の変数に再度`null`を割り当てることができるので、特に問題はありません。
...
- is_​array, is_​bool, is_​double, is_​float, is_​int, is_​integer, is_​long, is_​null, is_​numeric, is_​object, is_​resource, is_​string
...
#### empty가 정의되지 않은지 확인하는데 쓰이지않는 이유
#### emptyが未定義の変数を確認のに使用されない理由
- `isset`, `??`, `=??`은 정의되지 않은 경우와 `null`인 경우 `true`이므로 어떤 변수가 정의되지 않음을 확인할 때는 `null`이 아닌지 확인하면 명확하게 정의되지 않음을 알 수 있다. 하지만 `empty`는 정의되지 않은 경우 뿐만 아니라 `0`, `""`, `[]`, `0.0`, `"0"`, `null`, `false` 등 여러 값이 아닌지 모두 확인해야 변수가 정의되지 않았음을 명확하게 확인할 수 있으므로 변수의 존재와 비존재의 확인으로는 보통 쓰이지 않는다.
- `isset`, `？？`, `=??`は未定義の場合と`null`の場合は`true`を返すため、ある変数が未定義かどうかを確認するには`null`ではないかを確認すると、はっきりと未定義かどうかが分かります。 しかし、`empty`は未定義の場合だけでなく、`0`, `""`, `[]`, `0.0`, `"0"`, `null`, `false`などさまざまな値でないかをすべて確認してからこそ変数が未定義であることを確実に確認できます。そのため、変数の存在と非存在の確認としては一般的に使用されません。
- 또한 앞서 변수가 정의되지 않은 경우 할당을 위해서 `isset`, `??`, `=??`을 사용하자고 하였다. 이 때 `isset`의 결과가 `false`이고 변수가 정의되어 있을 때는 `null`인 경우만 생각하면 된다.
- また、先に「変数が未定義の場合は、割り当てのために`isset`, `??`, `=??`を使いましょう」と書きました。この時、`isset`の結果が`false`で変数が定義されている時は`null`の場合だけを考えれば十分です。
- 각 타입의 빈 값이 유효하게 할당된 값이라서 비할당을 나타내는데 사용할 수 없는 경우, 비할당을 상태를 추가하기 위해서 `null` 타입을 추가한다. `null`이 아닌 각 타입의 빈값을 사용하는 것은 비할당을 나타내는데 하나의 타입을 사용해도 충분한 경우일 때 사용하는 것으로 비할당의 양상을 나타내는 타입의 수를 줄인다.
- 各タイプの空の値が有効に割り当てられた値であるため、非割り当てを表すのに使用できない場合、非割り当ての状態を追加するために`null`タイプを追加します。 `null`ではない各タイプの空の値を使用することは、非割り当てを示すのに、1つのタイプを使用しても十分な場合に使用することで、非割り当ての様相を示すタイプの数を減らします。
- `null`이 할당된 변수에 디폴트 값으로 각 타입의 빈값을 할당하여 할당과 비할당을 구분하는 것은 `null`이 비할당으로 사용되는 가능성을 줄인 것으로 타입을 하나로 하여 모호함을 줄인다. 하지만 `empty`의 경우 통과 될 수 있는 값이 여럿이기 때문에 비할당 상태의 가짓수를 늘릴 가능성이 있다.
- `null`が割り当てられた変数にデフォルト値として各タイプの空の値を割り当て、割り当てと非割り当てを区分することは、`null`が非割り当てとして使用される可能性を減らしたもので、タイプを一つにして曖昧さを減らします。しかし、`empty`の場合、通過できる値が多数あるため、非割り当ての場合の数を増やす可能性があります。
- 예를 들어 변수에서 `0`이란 값을 비할당으로 사용하고 있었는데 `empty`가 `true`라서 디폴트 값으로 `null`을 할당하는 경우, 비할당을 타입의 빈 값과 `null` 두 가지를 고려해야 하는 모호한 것으로 만들어 버릴 수 있다. 그에 반해 `isset`은 `null`이라서 `null`을 할당하므로 아무런 차이가 없거나 빈 값을 할당하여 비할당의 타입의 수를 줄일 수 있다.
- 例えば、変数で`0`という値を非割り当てとして使用していましたが、`empty`が`true`であるため、デフォルト値として`null`を割り当て当てる場合、非割り当てをタイプの空の値と`null`の2つを考慮しなければならない曖昧なものにしてしまう可能性があります。それに対し、`isset`は`null`なので`null`を割り当てるので、何の違いもないか空の値を割り当てることで、非割り当てタイプの数を減らすことができます。

#### empty의 사용을 추천하지 않는 이유
#### emptyの使用をお勧めしない理由
- `empty`는 빈 값을 확인하는데 사용될 수 있지만, `empty`는 값이 존재할 경우 각 타입의 불리언 자료형의 타입 변환된 값이 `false`인 경우 `true`를 반환한다. php의 타입 저글링에 의해서 문자열 0(`"0"`)은 불리언으로 변환 될 경우 `false`가 되므로 `empty`는 `"0"`를 비어 있는 것으로 간주한다. 문자열의 빈값 `""`만을 구분하지 못해서 수로 구성된 문자열을 사용하는 경우에는 실수할 가능성이 있다. 이는 다른 언어와 동적 언어와 달리 php의 특수한 형변환 때문에 발생하는 것으로 `empty`를 잘 모르고 사용할 경우 실수할 가능성이 높아지므로 빈값을 확인할 때는 `=== 0`, `=== ""`, `=== []`을 사용할 것을 추천한다.
- `empty`は空の値を確認するために使用できますが、`empty`は値が存在する場合、各タイプのブール値型の型変換値が`false`の場合、`true`を返します。 phpのタイプジャグリングによって文字列0(`"0"`)は、ブールに変換される場合、`false`となるので、`empty`は`"0"`を空とみなします。 文字列の空の値`""`だけを区別できず、数で構成された文字列を使用する場合は、ミスをする可能性があります。 これは他の動的言語とは異なり、phpの特殊な型変換によって発生するもので、`empty`をよく知らずに使用するとミスする可能性が高くなるので、空の値を確認する際は`=== 0`, `=== ""`, `=== []`を使用することをお勧めします。

#### empty를 사용했다면?
#### emptyを使ったら？
- `empty`를 사용했다면 해당 변수는 단일 타입으로 사용되며 비할당을 빈 값으로 하는 변수라고 가정해도 아무런 문제가 없도록 짜여져야 한다. 그렇지 않다면 `empty`가 갖는 값이 어떤 `falsy`의 값인지 확인하기 하기 위해 맥락 파악 및 디버깅을 해야 하는 수고가 수반된다.
- `empty`を使用した場合、該当変数は単一タイプで使用され、非割り当てを空の値とする変数と仮定しても何の問題もないように組まなければなりません。 そうでない場合は、`empty`が持つ値がどの`falsy`の値かであるかを確認するために、コンテキスト把握およびデバッグを行う手間が伴います。
- 또는 복수의 타입을 사용하는 경우 타입 각각의 디폴트 값을 확인할 것 대신, `empty`로 어떤 타입의 디폴트값이 사용되고 있다는 것을 알 수 있다. 예를 들어 변수가 문자열과 수 타입로 정의가 되어 있다고 할 때 `$var === 0 || $var === ""`를 사용하는 것은 `empty`를 사용하는 것 보다는 명확하지만 코드의 길이가 너무 길어지기 때문에 간단히 `empty`를 사용하는 것이다. 프레임워크 등 코드 사용자의 편의를 위해서 여러 타입으로 쓸 수 있는 방식을 지원하는 경우가 아닌 한, 개인적으로는 거의 사용하지 않고 있다. 이 부분에 대해서는 각 프로젝트나 부서별로 코딩 스타일을 정하는 것을 추천한다.
- もしくは、複数のタイプを使用する場合、各タイプのデフォルト値を確認するのではなく、どのタイプのデフォルト値が使用されているかを`empty`で判断できます。例えば、変数が文字列と数値のタイプで定義されている場合、`$var === 0 | $var === ''`を使用することは明確ですが、コードが長くなるので、簡単にemptyを使うことがあります。フレームワークなどが利用者の利便性のために複数のタイプをサポートする場合ではなく、一般的な開発領域では`null`ではなく、必ず複数のタイプを使用するケースはまれです。個人的には、古いコードやライブラリを使用しない限り、あまり使用していません。この部分については、各プロジェクトや部署ごとにコーディングスタイル決めることをお勧めします。
...
- `isset`을 사용할 때는 정의되지 않은 값을 확인하고 로직을 실행하지 않는 방식으로 사용하는 것이 아니라, 아직 정의되지 않았을 때 값을 정의하기 위해 변수를 확인하는 용도 또는 정의되지 않은 값으로 비할당이 된 경우 정의된 값의 비할당 값을 설정하는 용도로 사용하자.
- `isset`を使用する際は、定義されていない変数を確認してロジックを実行しない方式で使用するのではなく、まだ定義されていないときに値を定義するために変数を確認する用途や、定義されていない値で非割り当てになった場合、定義された値の非割り当て値を設定する用途で使用しましょう。
