...
- そして、`null`は扱うためのデータとして作ることは一般的に考えられません。`null`の代わりに、特定の状態を示すために`boolean`タイプの変数を使用する方式でコードを作ります。デフォルトや非割り当ての状態を示すたに`null`を使用するコードを作成すると、コードを誤って組んでいる可能性が高いです。
