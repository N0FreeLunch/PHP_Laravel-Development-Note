...
- 非常に多くのphpロジックでは、`isset`を乱用していると思います。 どんなケースで`isset`を使用し、`isset`をなぜむやみに使用してはならないのか、その理由を理解してみよう。
- 불필요하게 `isset`을 사용한 코드를 볼 때 마다 불편한 느낌이 있었고 그 이유를 명확히 밝혀 `isset`을 남발하는 코드의 사용을 최소화하기 위한 목적으로 작성하였다.
- 不必要に`isset`を使ったコードを見るたびに不便な感じがあり、その理由を明確にして`isset`を乱発するコードの使用を最小化するためにこの記事を作成しました。

### 좋은 코드란 무엇인가?
### 良いコードは何か？
- 좋은 코드란 해석의 다양성을 줄이는 것이다. 해석의 가능성을 줄이는 것은 코드를 빠르고 명확하게 이해할 수 있도록 한다.
- 良いコードとは、解釈の多様性を減らすことです。 解釈の可能性を減らすことは、コードを迅速かつ明確に理解できるようにします。
...
- 例えば、phpで`const CONSTANT_VALUE='不変値'`と宣言された値を使用すると、このバリューは固定値であり、変更できないことがわかります。 変数とは異なり、定数は固定されたバリューであり、この値が最初に宣言された部分での値と異なることを考える必要がないので、変数の変更過程を推測する必要はありません。 定数を使用することは、値が変更される可能性の判断を減らすことで、解釈の多様性を減らすことです。
