...
2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てしません。単に割り当てられていないため未定義の変数が伝達される場合、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に応じる非割り当ての値をデフォルト値として代入し、非割り当てが未定義の変数になることを防止する。(前のコードの実行で必ず生成され伝達されるべき値は、未定義になって、ロジックの実行過程でエラーを発生させることが妥当であるため、デフォルト値を割り当てません。これに関しては後の例のコードの必須・オプショナルメンバー部分をご参考ください。)
...
1. 
    1. 変数を宣言する際、変数のデフォルト値を指定する。 変数の値が複数の条件によって変わる場合は、メソッドや関数などの戻り値を使用して変数の値を受け取るようにする。
    2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てしません。単に割り当てられていないため未定義の変数が伝達される場合、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に応じる非割り当ての値をデフォルト値として代入し、非割り当てが未定義の変数になることを防止する。
    3. ロジックを展開するのに必要な変数がすべて集まった後、ロジックの流れを表すコードを作成する。
2. 
