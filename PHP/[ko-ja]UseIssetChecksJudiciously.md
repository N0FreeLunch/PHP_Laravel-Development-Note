...
- `isset`을 사용할 때는 정의되지 않은 값을 확인하고 로직을 실행하지 않는 방식으로 사용하는 것이 아니라, 비할당으로 정의되지 않은 값에 각 변수에 맞는 비할당 값이 설정되도록 확인 및 기본값을 설정하는 용도로 사용하자.
- `isset`を使用する際は、定義されていない変数を確認してロジックを実行しない方式で使用するのではなく、非割り当てと定義されていない値に、各変数に適した非割り当ての値が設定されるように確認およびデフォルト値を設定する目的で使用しましょう。
