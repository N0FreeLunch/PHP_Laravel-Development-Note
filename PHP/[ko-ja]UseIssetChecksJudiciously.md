...
2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てず、ただ割り当てられていないため未定義の値が伝達される場合は、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に合わせて非割り当ての値をデフォルト値として代入し、非割り当てを表す値が未定義の場合になることを防止します。 （以前のコードの実行で必ず生成され伝達されるべき値は、未定となって後のロジックの実行過程でエラーを発生させることが妥当であるため、デフォルト値を割り当てません。 この部分は後の例題コードの必須・オプショナルメンバー部分をご参考ください。）
...
1. 
    1. 変数を宣言する際、変数のデフォルト値を指定する。 変数の値が複数の条件によって変わる場合は、メソッドや関数などの戻り値を使用して変数の値を受け取るようにする。
    2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てず、ただ割り当てられていないため未定義の値が伝達される場合は、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に合わせて非割り当ての値をデフォルト値として代入し、非割り当てを表す値が未定義の場合になることを防止します。 
    3. ロジックを展開するのに必要な変数がすべて集まった後、ロジックの流れを表すコードを作成する。
2. 
...
- `isset`을 사용할 때는 정의되지 않은 값을 확인하고 로직을 실행하지 않는 방식으로 사용하는 것이 아니라, 비할당으로 정의되지 않은 값에 각 변수에 맞는 비할당 값이 할당되도록 확인 및 기본값을 설정하는 용도로 사용하자.
- `isset`を使用する際は、定義されていない変数を確認してロジックを実行しない方式で使用するのではなく、非割り当てと定義されていない値に、各変数に適した非割り当て値が割り当てられるように確認およびデフォルト値を設定する目的で使用しましょう。
