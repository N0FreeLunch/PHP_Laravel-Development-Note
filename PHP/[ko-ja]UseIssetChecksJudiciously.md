...
2. 앞의 코드의 실행에서 반드시 전달되어야 하는 값의 경우에는 디폴트 값을 할당하지 않고, 그저 할당되지 않아 정의되지 않는 값이 전달되는 경우에는 `isset`, `??`, `??=` 의 문법을 사용하여 각 변수의 역할에 맞게 비할당에 해당하는 값을 디폴트 값으로 넣어주어, 비할당을 나타내는 값이 정의되지 않은 경우가 되는 것을 방지한다. (이전의 코드의 실행에서 반드시 생성되어 전달되어야 하는 값은, 미정의 되어 뒤의 로직의 실행 과정에서 에러를 발생 시키는 것이 타당하기 때문에 디폴트 값을 할당하지 않는다. 이 부분은 뒤의 예제 코드의 필수, 옵셔널 멤버 부분을 참고하도록 하자.)
...
2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てしません。単に割り当てられていないため未定義の値が伝達される場合、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に応じる非割り当ての値をデフォルト値として代入し、非割り当てが未定義の変数になることを防止する。(前のコードの実行で必ず生成され伝達されるべき値は、未定義になって、ロジックの実行過程でエラーを発生させることが妥当であるため、デフォルト値を割り当てません。これに関しては後の例のコードの必須・オプショナルメンバー部分をご参考ください。)
...
1. 
    1. 변수를 선언할 때 변수의 기본 값을 지정한다. 변수가 여러 조건에 따라 값이 달라진다면 메소드나 함수 등의 반환 값으로 변수 값을 받도록 하자.
    2. 앞의 코드의 실행에서 반드시 전달되어야 하는 값의 경우에는 디폴트 값을 할당하지 않고, 그저 할당되지 않아 정의되지 않는 값이 전달되는 경우에는 `isset`, `??`, `??=` 의 문법을 사용하여 각 변수의 역할에 맞게 비할당에 해당하는 값을 디폴트 값으로 넣어주어, 비할당을 나타내는 값이 정의되지 않은 경우가 되는 것을 방지한다. 
    3. 로직을 전개하는데 필요한 변수들이 다 모인 이후, 로직의 흐름을 나타내는 코드를 작성하도록 하자.
2. 
...
1. 
    1. 変数を宣言する際、変数のデフォルト値を指定する。 変数の値が複数の条件によって変わる場合は、メソッドや関数などの戻り値を使用して変数の値を受け取るようにする。
    2. 前のコードの実行で必ず伝達されるべき値の場合はデフォルト値を割り当てしません。単に割り当てられていないため未定義の値が伝達される場合、 `isset`, `??`, `??=` の文法を使用して、各変数の役割に応じる非割り当ての値をデフォルト値として代入し、非割り当てが未定義の変数になることを防止する。
    3. ロジックを展開するのに必要な変数がすべて集まった後、ロジックの流れを表すコードを作成する。
2. 
...
위의 작성 단위는 한 단위의 로직을 의미하며 로직의 단위가 A, B, C, D가 있고 로직에서 쓰이는 변수의 코드로 a, b, c, d가 있을 때 a b c d A B C D의 나열이 아닌 a A b B c C d D의 나열입니다. 이렇게 로직이 여럿으로 나뉘는 경우는 코드를 이어서 쓰기 보다는 함수나 메소드의 스코프에서 변수가 사용될 수 있도록 로직을 분리하거나 클래스로 서로의 로직을 분리해서 작성하는 방법을 고려하는 것이 좋습니다.

上記の作成単位は、一つのロジック単位を指し、ロジックの単位にA、B、C、Dが存在し、ロジックで使用される変数のコードにおいて「a, b, c, d」がある場合、「a b c d A B C D」の順序ではなく「a A b B c C d D」の順序で並んでいます。このようにロジックが多数に分かれる場合は、コードを書き続けるのではなく、関数やメソッドのスコープで変数を使用できるようにロジックを分離したり、クラスでロジックを分離する方法を考慮するのが良いでしょう。
...
- `empty(대상)` 함수는 `(fn($var) => !isset($var) || $var == false)(대상)` 코드와 같다.
...
- 또한 앞서 변수가 정의되지 않은 경우 할당을 위해서 `isset`, `??`, `=??`을 사용하자고 하였다. 이 때 `isset`의 결과가 `false`이고 변수가 정의되어 있을 때는 `null`인 경우만 생각하면 된다. 각 타입의 빈 값이 유효하게 할당된 값이라서 비할당을 나타내는데 사용할 수 없는 경우, 비할당을 상태를 추가하기 위해서 `null` 타입을 추가한다. `null`이 아닌 각 타입의 빈값을 사용하는 것은 비할당을 나타내는데 하나의 타입을 사용해도 충분한 경우일 때 사용하는 것으로 비할당의 양상을 나타내는 타입의 수를 줄인다. `null`이 할당된 변수에 디폴트 값으로 각 타입의 빈값을 할당하여 할당과 비할당을 구분하는 것은 `null`이 비할당으로 사용되는 가능성을 줄인 것으로 타입을 하나로 하여 모호함을 줄인다. 하지만 `empty`의 경우 통과 될 수 있는 값이 여럿이기 때문에 비할당 상태의 가짓수를 늘릴 가능성이 있다. 예를 들어 변수에서 `0`이란 값을 비할당으로 사용하고 있었는데 디폴트 값으로 `null`을 할당하여 비할당을 타입의 빈 값과 `null` 두 가지를 고려해야 하는 모호한 것으로 만들어 버릴 수 있다.
- また、先に「変数が未定義の場合は、割り当てのために`isset`, `??`, `=??`を使いましょう」と書きました。この時、`isset`の結果が`false`で変数が定義されている時は`null`の場合だけを考えれば十分です。 
各タイプの空の値が有効に割り当てられた値であるため、非割り当てを表すのに使用できない場合、非割り当ての状態を追加するために`null`タイプを追加します。 `null`ではない各タイプの空の値を使用することは、非割り当てを示すのに、1つのタイプを使用しても十分な場合に使用することで、非割り当ての様相を示すタイプの数を減らします。 `null`が割り当てられた変数にデフォルト値として各タイプの空の値を割り当て、割り当てと非割り当てを区分することは、`null`が非割り当てとして使用される可能性を減らしたもので、タイプを一つにして曖昧さを減らします。しかし、`empty`の場合、通過できる値が多数あるため、非割り当ての場合の数を増やす可能性があります。例えば、変数で`0`という値を非割り当てとして使用していましたが、デフォルト値として`null`を割り当て、非割り当てをタイプの空の値と`null`の2つを考慮しなければならない曖昧なものにしてしまう可能性があります。
