...
2. 혹시 변수에 값이 정의되지 않은 경우 `isset`, `??`, `??=` 의 문법을 사용하여 비할당에 해당하는 값을 변수의 디폴트 값으로 정한다. (경우에 따라 변수가 정의되지 않은 경우 일부러 에러가 발생하도록 해야 하는데 이 때는 디폴트 값을 할당하지 않는다. 예제 코드의 옵셔널 멤버 부분을 참고하자.)
...
2. 変数が値で初期化されていない場合は、 `isset`、`??`、`??=` の構文を使用して、変数が未定義であるかどうかを確認し、未定義の場合は非割り当てに該当する値をデフォルト値として定める。（場合によっては、変数が定義されていない場合にわざとエラーが発生するようにする必要がありますが、その際にはデフォルト値を割り当てません。例題コードのオプショナルメンバーの部分をご参考ください。）
...
1. 
    1. 변수를 선언할 때 변수의 기본 값을 지정한다. 변수가 여러 조건에 따라 값이 달라진다면 메소드나 함수 등의 반환 값으로 변수 값을 받도록 하자.
    2. 혹시 변수에 값이 정의되지 않은 경우 `isset`, `??`, `??=` 의 문법을 사용하여 비할당에 해당하는 값을 변수의 디폴트 값으로 정한다. (경우에 따라 변수가 정의되지 않은 경우 에러가 발생하도록 해야 하는데 이 때는 디폴트 값을 할당하지 않는다. 예제 코드의 옵셔널 멤버 부분을 참고하자.)
    3. 로직을 전개하는데 필요한 변수들이 다 모인 이후, 로직의 흐름을 나타내는 코드를 작성하도록 하자.
2. 
