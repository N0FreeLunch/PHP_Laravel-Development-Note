...
- 또한 앞서 변수가 정의되지 않은 경우 할당을 위해서 `isset`, `??`, `=??`을 사용하자고 하였다. 이 때 `isset`의 결과가 `false`일 때 변수가 정의되어 있더라도 `null`인 경우만 생각하면 된다. `null`은 각 타입의 빈 값이 유효하게 할당된 값이라서 비할당을 나타내는데 사용할 수 없는 경우, 비할당을 상태를 추가하기 위해서 `null` 타입을 추가한다. `null`이 아닌 각 타입의 빈값을 사용하는 것은 비할당을 나타내는데 하나의 타입을 사용해도 충분한 경우일 때 사용하는 것으로 비할당의 양상을 나타내는 타입의 수를 줄인다. `null`이 할당된 변수에 디폴트 값으로 각 타입의 빈값을 할당하여 할당과 비할당을 구분하는 것은 `null`이 비할당으로 사용되는 가능성을 줄인 것으로 아무런 문제가 되지 않느다. 하지만 `empty`의 경우 통과 될 수 있는 값이 여럿이기 때문에 비할당의 가짓수를 늘릴 가능성이 있다. 예를 들어 변수에서 `0`이란 값을 비할당으로 사용하고 있었는데 디폴트 값으로 `null`을 할당하여 비할당을 타입의 빈 값과 `null` 두 가지를 고려해야 하는 모호한 것으로 만들어 버릴 수 있다.
- また、先に変数が未定義の場合は、割り当てのために`isset`, `??`, `=??`を使いましょう」と書きました。この場合、`isset`の結果が`false`の場合、変数が定義されていても`null`の場合だけを考えれば十分です。 
`null`は、各タイプの空の値が有効に割り当てられた値であるため、非割り当てを表すのに使用できない場合、非割り当ての状態を追加するために`null`タイプを追加します。 `null`ではない各タイプの空の値を使用することは、非割り当てを示しますが、1つのタイプを使用しても十分な場合に使用することで、非割り当ての様相を示すタイプの数を減らします。 `null`が割り当てられた変数にデフォルト値として各タイプの空の値を割り当て、割り当てと非割り当てを区分することは、`null`が非割り当てとして使用される可能性を減らしたもので、何の問題にもなりません。しかし、`empty`の場合、通過できる値が多数あるため、非割り当ての場合の数を増やす可能性があります。例えば、変数で`0`という値を非割り当てとして使用していましたが、デフォルト値として`null`を割り当て、非割り当てをタイプの空の値と`null`の2つを考慮しなければならない曖昧なものにしてしまう可能性があります。

#### empty의 사용을 춫천하지 않는 이유
#### emptyの使用をお勧めしない理由
