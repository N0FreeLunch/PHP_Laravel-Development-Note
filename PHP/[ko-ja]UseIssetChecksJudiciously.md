...
2. 変数が値で初期化されていない場合は、 `isset`、`??`、`??=` の構文を使用して、変数が未定義であるかどうかを確認し、未定義の場合は非割り当てに該当する値をデフォルト値として定める。（場合によっては、変数が定義されていない場合にエラーが発生するようにする必要がありますが、その際にはデフォルト値を割り当てません。例題コードのオプショナルメンバーの部分をご参考ください。）
