...
- 단일 책임이란 말의 의미가 모호하긴 하지만, 하나의 기능을 중심으로 클래스를 설계한 것을 단일 책임을 가진 클래스라 할 수 있다. 위의 클래스는 운전자에게 제공될 자동차의 인터페이스를 나타낸 것으로 도메인에 명확하게 인터페이스가 규정된 경우에 해당한다. 인터페이스가 거의 고정된 형태로 큰 변경사항이 없는 것이기 때문에 위와 같은 클래스를 만든 것이다. 만약 고정된 인터페이스가 아니라 도메인의 개념 변경의 가능성이 높다면 위와 같이 하나의 클래스가 여러 기능을 갖는 것은 추천되지 않는다.

### 메소드명과 클래스명
- 위의 클래스는 운전자에게 제공할 자동자 조작의 인터페이스를 제공하는 것이다. 자동차의 인터페이스는 익히 잘 일려진 인터페이스이다. 하지만 이에 대한 인터페이스르 잘 모른다면 클래스명을 `Car`라고만 해서는 알 수 없다. `CarInterfaceForDriver`라는 명칭으로 운전자에게 제공할 자동차의 인터페이스 집합이라는 의미를 알려주는 클래스명이 더 좋다.
- 메소드명은 어떤 값을 받는지를 알려주기 보다는 어떤 목적을 위해서 값을 받는지 알려주는 이름을 정하는 것이 좋은데 `handleLeft`라는 이름은 왼쪽으로 자동차의 이동 방향을 설정한다는 의미를 가지고 있다. 어떤 값을 받는지에 초점을 맞추면 `setLeftAngle`이 되겠지만, `handleLeft`라는 이름은 무엇을 하는지를 알려 주고 `$angle`이란 변수명을 통해서 무엇을 받는지 알 수 있다. 메소드는 파라메터를 통해서 무엇을 받는지 알 수 있기 때문에 어떤 목적을 위해서 메소드를 사용하는지를 알 수 있도록 메소드명을 정의하도록 하자.
