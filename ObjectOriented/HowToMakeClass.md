...
### 단일 책임 원칙 이해하기
- 앞서 객체를 만들 때는 책임의 범위를 줄인다고 하였다. 그리고 책임의 범위를 최대한 줄인 단일 엑션 클래스를 만드는 것이 좋다고 하였다. 단일 책임 원칙은 객체를 만들 때 하나의 책무를 수행하도록 만들어야 한다는 것이다. 왜 단일 책임 원칙으로 만드는지 그 이유를 알아 보자.
> 책임을 변경하려는 이유로 정의하고, 어떤 클래스나 모듈은 변경하려는 단 하나 이유만을 가져야 한다고 결론 짓는다. <sup>[wiki](https://ko.wikipedia.org/wiki/%EB%8B%A8%EC%9D%BC_%EC%B1%85%EC%9E%84_%EC%9B%90%EC%B9%99)</sup>
- 좀 더 자세한 내용은 [SingleReponsibilityPrinciple](https://blog.cleancoder.com/uncle-bob/2014/05/08/SingleReponsibilityPrinciple.html) 부분을 참고하자.
- 한 클래스가 서로 관련성이 떨어지는 여러 기능을 담고 있다면, 하나의 기능을 변경할 때 다른 기능의 손상될 가능성이 생긴다. 하나의 기능이 변경되더라도 개념적으로 서로 관련성이 떨어지는 기능에 영향을 주지 않게 하기 위해서 단일 책임의 원칙에 따라 클래스를 만든다.
