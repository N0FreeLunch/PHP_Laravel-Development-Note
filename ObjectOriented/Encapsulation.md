...
- 하나의 메소드 명이 나타내는 기능은 1대1 대응이 되어야 한다. 메소드를 실행했을 때 메소드 이름이 나타내는 의미 보다 훨씬 더 많은 영향력이 있다면 유저가 메소드를 때 객체에 미치는 영향력을 파악하기 어렵다. 곧 객체를 통제하기 어렵게 되기 때문에 메소드가 가진 이름 보다 더 많은 영향을 가졌다면 여러 개의 메소드로 분리해서 가능한 메소드의 기능에 대응하는 이름을 지어 주는 것이 좋다.
