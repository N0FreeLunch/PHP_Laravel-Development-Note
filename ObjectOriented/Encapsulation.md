...
-  앉아라는 소리를 듣고 뇌가 이를 신경에서 처리해서 뇌의 판단을 통해서 근육의 움직임을 조절 하는 것은 알 필요가 없다.
...
## 인터페이스란?
- 여기서 말하는 인터페이스는 OOP 언어의 문법 인터페이스를 의미로 설명하지 않으며 개념적인 인터페이스로 설명한다.
- 인터페이스란 객체 간의 상호작용을 정의하는 규약을 의미한다.
- 인터페이스의 구성은 일반적으로 지시와 결과로 이뤄진다.

### 인터페이스의 예시
- 주인이 '앉아'라고 했을 때 (명령) 개가 앉는 것이 (결과) 인터페이스이다.
- 주인이 인형을 던졌을 때 (명령) 개가 인형을 물어 오는 것이 (결과) 인터페이스이다.
- 주인이 개를 데이고 산책을 나갔을 때 (명령) 알아서 걸어서 따라오는 것이 (결과) 인터페이스이다.

### 인터페이스는 객체를 향한 명령이며 명령에 대한 결과를 기대한다.
...
- 객체를 사용하는 것이 인터페이스의 본질이기 때문에 문법적인 인터페이스는 public 메소드만으로 구성된다. public 멤버의 경우 객체를 사용하는 방법으로 제공되는 것이 아니기 때문에 객체의 사용설명서가 되어야 하는 인터페이스의 요소에 적합하지 않다.
...
- private 또는 protected로 구성된 메소드는 객체의 외부에서 접근하거나 사용할 수 없는 문법적인 제약을 갖는다.
...
- 객체를 만든다는 것은 객체에 대한 사용 설명서를 퍼블릭 메소드로 만든다는 의미를 가지고 있다. 퍼블릭 메소드로 제공될 것은 퍼블릭 메소드만 보고서도 직관적으로 어떤 메소드를 사용해야 객체가 제공하는 기능을 사용할 수 있는 것인지 알 수 있도록 의미를 부여하도록 한다.

### 조합성
- 메소드가 여러 인자를 가지게 되면 메소드의 역할을 직관적으로 이해하기 어렵고 인자의 순서도 헷갈리기 쉽다. 따라서 여러개의 인자를 받아야 한다면 여러 메소드로 분리해서 이를 받는 것이 좋다.
- 하나의 메소드 명이 나타내는 기능은 가능하면 1대1 대응이 되어야 한다. 메소드를 실행했을 때 메소드 이름이 나타내는 의미 보다 훨씬 더 많은 영향력이 있다면 유저가 메소드를 때 객체에 미치는 영향력을 파악하기 어렵다. 곧 객체를 통제하기 어렵게 되기 때문에 메소드가 가진 이름 보다 더 많은 영향을 가졌다면 여러 개의 메소드로 분리해서 가능한 메소드의 기능에 대응하는 이름을 지어 주는 것이 좋다.
...
- 유비쿼터스 언어란 컴퓨터 시스템을 설계하기 위해서 시스템을 만들어 나가는 다양한 참여자들이 서로 공유할 수 있는 용어로 정의한 용어들로 구성되며 비즈니스 영역의 로직을 논리적인 관계가 맞아들어가도록 구체적으로 하나하나 잘 풀어 설명한 것이다.
